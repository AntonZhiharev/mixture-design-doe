...
#:
#: iter84: снижен с 24 000 до 12 000. Замер живой ПВХ-сессии (43 реплики,
#: ~100 тыс. символов) показал, что при 24 000 бюджет НИКОГДА не срабатывал:
#: переписка занимала ~23,7 тыс. токенов, то есть каждый вопрос тащил к модели
#: весь разговор целиком — вместе с системным промптом (19,5 тыс. символов) и
#: JSON-схемами 26 инструментов (16,6 тыс. символов) выходило ~30 тыс. токенов
#: НА КАЖДЫЙ ход, и почти всё это — снимки, которые модель обязана
#: перезапрашивать инструментами (канон промпта: «утверждение о геометрии без
#: вызова инструмента — выдумка»).
#:
#: 12 000 токенов — это ~15–20 последних реплик такого разговора: рабочая ветка
#: обсуждения цела, а отсечённое НЕ теряется и не замалчивается (явная пометка
#: `[сессия]`, полная переписка в `session.json` проекта, A0.6). Число вынесено
#: в константу, чтобы его можно было поднять для длинного разбора одной правкой.
...
FOCUS_SECTIONS: Tuple[FocusSection, ...] = (
    FocusSection(
        key="setup", title="🧩 Настройка области опытов",
        doing=("задаёт компоненты смеси (Σ=1), процесс-оси и отклики — "
               "геометрия проекта только формируется"),
        asks=(("Почему границы не такие, как я вводил",
               "Почему действующие границы отличаются от того, что я вводил?"),
              ("Можно строить план?",
    ...
    FocusSection(
        key="seed", title="🌱 Стартовый план опытов",
        doing=("предлагает стартовый план и вносит измеренные отклики — "
               "до фиксации плана база проекта пуста"),
    ...
    FocusSection(
        ...
        doing=("сверяет рецепт с шагом весов: расчётная навеска против "
               "фактической"),
    ...
    FocusSection(
        ...
        doing=("смотрит уже измеренные точки проекта (источник, отклики)"),
    ...
    FocusSection(
        ...
        doing=("предлагает точки, вносит отклики и добавляет их в общую базу"),
        asks=(("Эта рецептура в границах?",
               "Эта рецептура попадает в границы геометрии?"),
              ("Что изменится, если…",
               "Что изменится, если сузить {node}?"))),
    FocusSection(
        key="evolution", title="🧬 Изменение схемы",
        ...
        asks=(("Что изменится, если…",
              ...
              ("Почему диапазон не такой",
               "Почему диапазон {node} не такой, как я вводил?"))),
    FocusSection(
        key="screening", title="📊 Влияние компонентов на свойства",
        doing=("читает влияние состава по измеренному стартовому плану"),
        asks=(("Объясни эту ось", "Объясни эту ось: {node}"),
              ("Что в базе?", "Сколько точек в базе и что они покрывают?"))),
)
...
def run_turn(session: AssistantSession, ctx: Any, question: str, *,
    ...
    * ``images`` (iter68) — приложенные к ЭТОМУ вопросу изображения (sha256
    ...
    # iter84: ссылки на файлы расчёта пишутся В САМО сообщение ответа, а не
    # только в TurnResult. Прежде связь «ответ ↔ его график» жила в памяти
    # одного прогона Streamlit: показанные под ответом график и таблица
    # исчезали при первом же rerun и после перезапуска приложения — сами файлы
    # оставались в проекте, но из разговора пропадали, и человек видел
    # «ассистент построил график и потерял его». Артефакты считаются ДО
    # add_message, потому что порядок здесь несёт смысл: нельзя приложить к
    # сообщению список, которого ещё нет.
    session.add_message("assistant", res.text, model=res.model, web=res.web,
                        usage=res.usage, artifacts=list(res.new_artifacts))
