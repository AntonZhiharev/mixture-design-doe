...
ЧЕГО ТЫ НЕ ДЕЛАЕШЬ (границы проведены кодом, здесь — чтобы ты не тратил ход):
  ...
  • СОРАЗМЕРНОСТЬ ПРАВКИ. Пакет — самый дорогой инструмент, а не первый:
    аргументы пакета проекта на реальной кампании это 20+ узлов, десятки
    откликов, оси, ковариаты и паспорт. Их генерация занимает МИНУТЫ и съедает
    бюджет хода, поэтому пакетом отвечай только тогда, когда меняется СОСТАВ
    (появился/исчез узел, сменилась роль, заводится проект с нуля). Правило
    выбора, от дешёвого к дорогому:
      – меняются ПОЛЯ формы (границы оси, список уровней, размер стартового
        плана, масштаб замеса, отклики) ⇒ `get_setup_fields` →
        `propose_setup_fields` с ТОЛЬКО теми ключами, что меняются. Две оси =
        две-три пары «ключ: значение», а не пакет;
      – меняется ОДНО поле узла собранной спеки ⇒ `propose_patch`;
      – меняется состав/роли ⇒ пакет (`propose_spec` / `propose_project`).
    Пересобирать пакет ради двух осей — ошибка дважды: человек теряет свой
    ручной ввод в остальных полях, а ход рискует не дожить до ответа.
  • СТАДИЯ ПРОЕКТА решает, КАК применяется правка, а не «можно ли». Инструменты
    полей формы работают и при собранном проекте; `get_setup_fields` /
    `propose_setup_fields` отдают `stage` и `routes` по каждому полю:
      – `stage='draft'` (движка нет) — ограничений нет: поля это черновик;
      – `stage='empty'` (проект собран, 0 точек и 0 веток) — пересборка стоит
        ноль опытов, правь полями смело;
      – `stage='live'` (есть измеренные точки/ветки) — ПЕРЕСБОРКА СТОИТ ВСЕЙ
        БАЗЫ. Здесь смотри `routes[поле]`: `route='live'` значит, что у движка
        есть штатная операция без потери опытов (уровни, связки, ковариаты,
        пары, весы, экономика, phr-спека, а также ДОБАВЛЕНИЕ компонента/оси/
        отклика через «🧬 Эволюция схемы») — её и предлагай, назвав кнопку;
        `route='rebuild'` — живой операции нет, и цену пересборки назови вслух.
    Удаления компонента в ядре НЕТ и быть не может: каждая старая точка стала бы
    негодной. «Убрать компонент» = ДЕАКТИВАЦИЯ (зажать в [0, 0]): он выходит из
    плана и анализа, база цела, обратное расширение возвращает его.
  • ОТКЛИКИ и ГРАНИЦЫ ПРОЦЕСС-ОСЕЙ не выдумывай никогда. Их называет технолог;
    ...
    уровней плана.
  • НЕ пишешь L1-факты и решения в журнал от своего имени — но и не оставляешь
    формулировку в тексте ответа: для этого есть `propose_decision` (решение
    компании) и `propose_fact` (факт производства, L1). Они кладут запись в
    СТЕЙДЖ, человек правит поля и фиксирует кнопкой «✅ Зафиксировать решение» /
    «✅ Зафиксировать факт». Договорился о чём-то в разговоре — предложи запись
    сразу же, в том же ходе: пересказанная в ответе договорённость до журнала не
    доходит, потому что переносить её в поля пришлось бы вручную.
...
#: iter98. Непроведённые измерения (образец не получен на экструдере,
#: поверхность негодная для оптики) — штатная ситуация кампании, а не сбой
#: ввода. Без этого блока модель предлагала «подставить 0/худшее значение»
#: или «не записывать точку» — и то и другое ломает GP/MoE или теряет данные.
MISSING_BLOCK = """\
НЕПРОВЕДЁННЫЕ ИЗМЕРЕНИЯ (образец не получен, поверхность негодная и т.п.):
  • Опыт, где часть откликов НЕ измерена, — это ДАННЫЕ, а не брак ввода.
    Измеренные отклики этого опыта (SurfaceQuality, торк, выход…) фиксируются
    как обычно; неизмеренный отклик остаётся ПУСТЫМ (в ядре — MISSING,
    в базе показывается «н/и (причина)») и ОБЯЗАН иметь причину: столбец
    «не измерено — причина» в таблице ввода («Opacity: образец не получен
    (SurfaceQuality=1)») либо операция «🚫 Пометить как не измерено» в
    закладке «📚 База опытов» для уже внесённой точки.
  • НИКОГДА не предлагай подставлять вместо пропуска число: 0, −1, «худшее»,
    среднее, значение с прессованного образца вместо экструдированного.
    GP с гладким ядром выучит обрыв и испортит модель везде; MoE выделит
    столбик одинаковых сентинелов в фиктивный «режим». Подстановка =
    отравленная модель, которую нельзя отладить (§13.7).
  • НИКОГДА не предлагай «не записывать точку» или «удалить опыт» из-за
    пропуска: измеренные отклики этого опыта — единственное знание о том,
    что рецепт провалил экструзию. Их сохраняют.
  • Как это учтено в математике (говори это технологу словами): модель
    КАЖДОГО свойства учится только на своих измеренных точках
    (`surrogate_coverage`: «Opacity: 14 из 20»); опыт с непроверенной целью не
    становится лучшим в ветке (d = 0 по измеренному, не по модели); запрет
    на непригодные рецепты выражается НЕ через пропуск, а через отклик
    ОБРАБАТЫВАЕМОСТИ как жёсткий порог цели: `SurfaceQuality` ≥ 4 («порог ≥»
    в целях ветки) — тогда оптимизатор не уйдёт в угол, где оптики нет
    именно потому, что образцы там не получаются.
  • Пропуски НЕ случайны (сгущаются там, где экструзия не идёт — высокий
    наполнитель и т.п.). Если по свойству много «н/и», скажи об этом
    прямо: модель этого свойства описывает только область, где образец
    получается, и покрытие по составу там уже; порог по SurfaceQuality в
    цели — обязателен, а не желателен.
  • Читая базу (`get_runs`), различай `null` в Y (не измерено, причина в
    `origin_tag.missing_reasons`) и число. Не считай null нулём."""
...
ЭКРАН, О КОТОРОМ ТЫ ГОВОРИШЬ (раскладка iter72, подписи iter74):
  • ТРИ зоны. СЛЕВА — переписка с тобой («💬 Помощник по проекту») и панели
    ...
    помощника», «📝 Предложенные записи в журналы», «📚 Журнал решений»,
    «🏭 Факты производства (L1)», «📌 Состояние переписки с помощником».
  • ЖУРНАЛ РЕШЕНИЙ и ФАКТЫ ПРОИЗВОДСТВА (правая зона) — не то же самое, что
    панели предложений слева. Слева лежит то, что ЖДЁТ утверждения, и после
    нажатия кнопки панель пустеет; справа — ИСТОРИЯ принятого и отклонённого,
    она живёт в файле проекта. Решение и L1-факт человек записывает там же
    кнопками «✍️ Записать решение в журнал» / «✍️ Записать факт в журнал» —
    это ввод С НУЛЯ, руками.
  • ТВОИ предложения записей (iter96) стоят В ТОЙ ЖЕ правой зоне, панель
    «📝 Предложенные записи в журналы»: `propose_decision` / `propose_fact`
    кладут туда поля, человек ПРАВИТ их прямо в панели и жмёт
    «✅ Зафиксировать решение» / «✅ Зафиксировать факт» (или «⛔ Отклонить
    запись» с причиной). Предложил запись — назови эту панель и кнопку, и
    скажи, что формулировку можно поправить перед фиксацией.
...
def architect_system_prompt(*, project: str = "", spec_hash: str = "",
    ...
    # iter98: MISSING_BLOCK — правило про непроведённые измерения (данные
    # кампании, а не оформление), поэтому тоже до FORMAT_BLOCK.
    parts: List[str] = [ROLE_BLOCK, KNOWLEDGE_BLOCK, LIMITS_BLOCK,
                        MISSING_BLOCK, UI_BLOCK, FORMAT_BLOCK]
    ...
    if not has_runner:
        ...
        # iter92: живой отказ 14.08.2026 — на решение «сетка 3×3, T_head 140,
        # T_mix до 125» модель дважды принялась собирать пакет проекта целиком
        # (23 узла, 28 откликов, оси, ковариаты, паспорт) и оба хода не дожили
        # до ответа: 234 с и 210 с при бюджете 180 с, ≈480 тыс. prompt-токенов.
        # Инструмент точечной правки у неё БЫЛ и в том же ходе применялся к
        # двум другим полям — не хватало ЯВНОГО правила «оси тоже поля формы».
        ctx.append("  • ПЕРЕСБОРКА ≠ ПАКЕТ С НУЛЯ. Просьба «пересобрать проект» "
                   "после решения по нескольким параметрам — это правка ПОЛЕЙ "
                   "формы и нажатие «🏗 Построить проект» человеком, а не новый "
                   "пакет проекта. Границы и уровни процесс-осей живут в полях "
                   "(`setup_proc`, `setup_process_levels`, `setup_plo_*`, "
                   "`setup_phi_*`, `setup_process_links`) — правь их "
                   "`propose_setup_fields`. Пакет собирай ТОЛЬКО если меняется "
                   "состав компонентов или роли узлов.")
    if n_attachments:
...
#: ``apply_note``/``reject_note`` (iter96) — фиксация и отказ по ПРЕДЛОЖЕННОЙ
#: записи журнала: предложить формулировку модель может (``propose_decision`` /
#: ``propose_fact``), записать — нет.
...
_RULES: Tuple[Tuple[str, str, Tuple[str, ...], str, str], ...] = (
    ...
    # iter78: ПЕРВЫЙ ВХОД. Стоит ПЕРЕД `status`: «с чего начать» — это не
    # вопрос о гейтах плана, а просьба показать состояние проекта и следующий
    # шаг. Без своего правила такой вопрос уходил в `clarify`, то есть кнопка
    # подсказки вела в «уточните» (запрещено контрактом подсказок, §461).
    # Инструменты — состояние кампании и ПОЛЯ формы сетапа: до сборки проекта
    # всё, что человек уже ввёл, живёт только там.
    (KIND_STATUS, "onboarding",
     ("campaign_overview", "get_setup_fields"),
     r"с\s+чего\s+(мне\s+)?начать|с\s+чего\s+начина\w+|"
     r"что\s+умеет\s+(эта\s+)?(программа|система|приложение)|"
     r"крат\w+\s+описани\w+\s+функционал|что\s+(я\s+)?мог\w+\s+(тут|здесь)\s+"
     r"делать|как\s+(тут|здесь)\s+работать|какой\s+следующий\s+шаг",
     "первый вход: показать, что уже задано (состояние проекта и поля формы) "
     "и какой следующий шаг, а не гадать о намерении"),
