...
- `propose_seed` на непустой базе = `propose_augment` (maximin от
  ...
  ядра.
- `move_region` по process-осям для оракула в коде [0,1] область не сужает
  (`point_in_region` проверяет код, границы лишь переинтерпретируют его в
  реальные единицы). Окрестность дыры сужена по компонентам; для ручной
  кампании с реальными единицами это работает как задумано.
