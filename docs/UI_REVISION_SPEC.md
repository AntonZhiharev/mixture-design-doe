...
| Hard constraints (Adhesion, Opacity, `Pr(ΔE≤ΔE_max)`) | ✅ | ❌ | редактор целей знает только `max/min/target`; `set_branch_chance` нет |
...
| **P1.2** | iter43 | Виды целей «порог ≥/≤» (ramp = шум измерения) и `Pr(y≤max)≥1−α`; `set_branch_chance` + персистентность; показ `binding_report` |
