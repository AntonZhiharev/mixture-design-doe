...
* **раскрытие картинки на весь экран** (:func:`fullscreen_escape_css`, iter94) —
  плата за липкость: ``position: sticky`` создаёт stacking context, поэтому
  полноэкранная обёртка Streamlit оставалась внутри колонки и обрезалась ею
  (внешне «картинка просвечивает»); правило снимает липкость с той колонки, где
  раскрытие сейчас происходит;
...
# ----------------------------------------------------------------------
# Раскрытие картинки на весь экран внутри липкой зоны (iter94)
# ----------------------------------------------------------------------
#: Подписи кнопки «выйти из полноэкранного показа» в Streamlit 1.58. Кнопка
#: рисуется ТОЛЬКО в раскрытом состоянии (``t && !o && n && P(iW, {label:
#: `Close fullscreen`…})`` в бандле, где ``n`` = ``isFullScreen``), поэтому её
#: наличие в поддереве — единственный СТАБИЛЬНЫЙ признак «здесь сейчас раскрыто»,
#: доступный из CSS: сам признак живёт в React-состоянии, а класс раскрытой
#: обёртки — сгенерированный хеш emotion, на который цепляться нельзя.
#:
#: Две подписи, потому что чанки разных элементов пишут её по-разному:
#: ``Close fullscreen`` (``index.*.js``, ``PlotlyChart.*.js``) и
#: ``Exit fullscreen`` (``DeckGlJsonChart.*.js``, ``PlotlyChart.*.js``).
FULLSCREEN_EXIT_LABELS: Tuple[str, ...] = ("Close fullscreen", "Exit fullscreen")


def fullscreen_escape_css(*, min_width_px: int = STICKY_MIN_WIDTH_PX) -> str:
    """CSS, возвращающий раскрытой картинке весь экран (iter94).

    Чистая функция, как и :func:`sticky_zones_css`; отдельная от неё намеренно —
    это ДРУГОЕ решение (снятие липкости на время раскрытия), и смешивать их в
    одной строке значило бы прятать компромисс.

    Отказ (наблюдение технолога 14.08.2026): картинка из ответа помощника при
    раскрытии на весь экран показывалась «просвечивающей» — сквозь неё была видна
    рабочая область и инфо-панель.

    Причина — не в файле (png непрозрачны: RGBA с alpha=255, фон белый) и не в
    коде показа, а в сцепке НАШЕГО ``position: sticky`` (iter89) с полноэкранной
    обёрткой Streamlit. Проверенные факты о 1.58:

    * обёртка (``withFullScreenWrapper.*.js``) в раскрытом виде получает
      ``position: fixed``, ``top/left/right/bottom: 0``,
      ``background: theme.colors.bgColor``, ``overflow: auto`` и
      ``zIndex: theme.zIndices.fullscreenWrapper``;
    * ``zIndices.fullscreenWrapper`` = ``1e6 + 50`` = 1000050 — заведомо выше
      всего остального на странице;
    * базовый стиль колонки (``stColumn``) ``position`` НЕ задаёт: у стокового
      Streamlit обёртка лежит в статическом потоке, и её z-index работает
      относительно корня — поэтому раскрытие там закрывает страницу целиком.

    Что ломает липкость: ``position: sticky`` **всегда создаёт новый stacking
    context** (MDN, справка по ``position``). Значит z-index 1000050 перестаёт
    сравниваться с элементами страницы и действует только ВНУТРИ колонки, а
    ``max-height`` + ``overflow-y: auto`` этой же колонки вдобавок обрезают
    «полноэкранный» блок. Внешне это и выглядит как «картинка просвечивает».

    Лечение: пока в колонке есть кнопка выхода из полноэкранного показа
    (:data:`FULLSCREEN_EXIT_LABELS`) — снимаем с этой колонки и липкость, и
    ограничение высоты, и обрезку. Тогда обёртка снова считает z-index от корня
    и закрывает страницу, как в стоковом Streamlit. Липкость возвращается сама:
    кнопка исчезает из DOM вместе с раскрытием.

    Правило живёт в ТОМ ЖЕ ``@media``, что и липкость: ниже
    :data:`STICKY_MIN_WIDTH_PX` колонки не липкие, и отменять нечего.
    """
    sel = ", ".join(f'[data-testid="stColumn"]:has(button[aria-label="{label}"])'
                    for label in FULLSCREEN_EXIT_LABELS)
    return f"""<style>
@media (min-width: {min_width_px}px) {{
  {sel} {{
    position: static;
