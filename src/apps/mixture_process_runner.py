...
class MixtureProcessRunner:
    ...
    def set_project_economics(self, *, enabled: bool = True,
        ...
        ``prices`` — ``{компонент: цена за единицу массы}``; НЕПУСТОЙ словарь
        обязан покрывать ВСЕ mixture-компоненты полной схемы, иначе отказ (A0.6:
        молчаливый нуль в цене даёт заниженную себестоимость).

        iter85: ``prices=None``/``{}`` — ЛЕГАЛЬНОЕ состояние «ρ объявлена, цены
        ещё не заданы»: закупочная цена приходит из снабжения позже, чем
        собирается проект, и требовать её вперёд значило бы либо задерживать
        старт опытов, либо вписывать выдуманные нули. Это НЕ то же, что нулевая
        цена: пустой набор даёт ``project_price_vector() == []`` ⇒ ценовая нога
        не собирается вовсе (ветки рождаются техническими, себестоимость не
        считается), тогда как нуль означает «сырьё бесплатно» и в расчёт идёт.
        Цены доводятся потом штатным ``CampaignController.set_project_prices``
        (iter82) — база опытов при этом не трогается.
    ...
    def _set_component_prices(self,
        ...
        iter85: ПУСТОЙ набор (``None``/``{}``) — не «частичные цены», а отдельное
        состояние «цены ещё не заданы»: полнота проверяется только у непустого
        словаря. Пустой набор не даёт ценовой ноги вообще
        (``project_price_vector() == []``), поэтому занизить себестоимость
        нечему — считать её просто не начнут (ветки останутся техническими).
        Требовать цену ДО первого опыта нельзя: она приходит из снабжения
        позже, и гейт на сборке заставлял бы вписывать выдуманные числа.
        """
