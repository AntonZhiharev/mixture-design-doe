...
def campaign_ui_guide() -> Dict[str, Any]:
    """Карта единого UI кампании (§17): сетап → seed → ветки → рабочий стол →
    эволюция. ОДИН движок физики (MixtureProcessRunner), ветка — контейнер
    намерения, отклики вносятся вручную (§17.2/§17.4)."""
    return {
        "_note": "Карта единого потока кампании (§17). Один движок физики, ручной "
                 "ввод откликов; опирайся на неё для вопросов про кнопки/шаги.",
        "flow": {
            "1. Сетап (§17.4)": "закладка «🌱 Старт» рабочей области: панель "
                "«📁 Проект» (сохранить/загрузить) и форма «🆕 Новый проект» "
                "(компоненты смеси, процесс-параметры с границами, отклики) "
                "→ «🏗 Построить проект».",
            "2. Seed (§17.4)": "«🌱 Стартовый дизайн»: «📐 Предложить seed-дизайн» "
                "→ внести Y по всем свойствам → «💾 Зафиксировать seed». "
                "«🧪 Заполнить тестовыми» — демо-оракул (прогон без лаборатории).",
            "3. Ветки (§17.5)": "«➕ Создать ветку вручную»: мультицель "
                "(отклик/вид/диапазон/вес), опц. ценовая нога ρ + цены состава.",
            "4. Рабочий стол (§17.2)": "«🛠 Рабочий стол ветки»: «📐 Предложить "
                "точки» → внести Y → «💾 Долить измеренные» → §4-стоп.",
            "5. Эволюция (§16.2)": "«🧬 Эволюция схемы»: раскрыть ось/компонент, "
                "ввести отклик, подвинуть границы (миграция по явной политике).",
        },
    }
...
def campaign_system_prompt() -> str:
    ...
    return (
        ...
        "модель физики на проект (общие GP-суррогаты на каждое свойство); "
