...
def branch_money_explanation(runner, branch_id: str, *,
    ...
    if suppressed:
        ...
        text = (
            f"Отклик «{rho}» одновременно является целью качества и входом модели "
            "себестоимости изделия. Чтобы одна и та же неопределённость «{rho}» не "
            "учитывалась дважды, её информационная ценность засчитывается только в "
            "качественном критерии (desirability), а в денежном критерии остановки "
            "НЕ начисляется. Поэтому ожидаемый экономический эффект уточнения "
            "«{rho}» за раунд равен 0 ₽. Снижение себестоимости от выбора состава "
            "остаётся частью desirability, но отдельным денежным порогом остановки "
            "не является.").replace("«{rho}»", f"«{rho}»")
    else:
        reason = "price_input_alive"
        tail = (" Ожидаемый экономический эффект уточнения за плановый горизонт "
                f"≈ {economic_value:,.0f} ₽ (снижение себестоимости изделия × "
                "плановый объём выпуска)."
                if economic_value is not None else "")
        text = (
            f"Отклик «{rho}» входит только в модель себестоимости изделия "
            "(себестоимость = цена состава × «{rho}») и не является отдельной "
            "целью качества. Его информационная ценность УЧИТЫВАЕТСЯ в денежном "
            "критерии остановки: снижение неопределённости «{rho}» уменьшает "
            "ожидаемую себестоимость и засчитывается деньгами."
        ).replace("«{rho}»", f"«{rho}»") + tail
