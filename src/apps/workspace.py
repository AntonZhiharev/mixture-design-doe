...
* **высоты панелей** (:data:`WORKSPACE_HEIGHT`, :data:`CHAT_FEED_HEIGHT`) —
  лента диалога и рабочая область скроллятся НЕЗАВИСИМО: лента уходит вверх
  историей, рабочая область стоит на месте.
...
#: Высота рабочей области (px). Фиксированная — иначе смысл теряется: контейнер
#: со свободной высотой снова растягивает документ и возвращает общий скролл.
...
#: Пропорции колонок главного экрана: диалог слева, рабочая область справа
#: (эскиз пользователя). Диалог — узкая колонка, но с собственным скроллом.
MAIN_COLUMNS: Tuple[int, int] = (2, 5)
...
WHY: Dict[str, str] = {
    NEED_PROJECT: ("проект не собран: соберите его в «🆕 Новый проект» "
                   "(левая панель) или создайте демо-проект"),
    NEED_POINTS: ("стартовый дизайн ещё не измерен: зафиксируйте seed на "
                  "закладке «🌱 Старт»"),
...
WORKSPACE_TABS: Tuple[WorkspaceTab, ...] = (
    WorkspaceTab(
        key="start", title="🌱 Старт", focus="seed", needs=NEED_PROJECT,
        blurb=("Настройки проекта и стартовый дизайн (seed): предложить план, "
               "внести измеренные отклики, зафиксировать.")),
    WorkspaceTab(
        key="base", title="📚 База опытов", focus="base", needs=NEED_POINTS,
        blurb=("Общая база всех измеренных опытов (И-1): выгрузка в Excel, "
               "коррекция откликов, ковариаты прогона.")),
    WorkspaceTab(
        key="branches", title="🌿 Ветки", focus="branch", needs=NEED_POINTS,
        blurb=("Ветки проекта: роли откликов, цели, рабочий стол ветки, "
               "рецепт x*, undo и spawn. Второй ряд закладок — сами ветки.")),
    WorkspaceTab(
        key="screening", title="📊 Анализ", focus="screening",
        needs=NEED_POINTS,
        blurb=("Интерпретируемый скрининг по измеренной базе: влияние "
               "компонентов на свойства (Scheffé + ARD).")),
    WorkspaceTab(
...
def default_tab_key(*, has_project: bool, n_points: int = 0,
    ...
    Пока проекта нет — «Обзор» (единственная закладка без требований к
    состоянию); пока seed не измерен, работать можно только на «Старте»; после
    измеренного seed интерес человека уходит к веткам (там рабочий стол), а не
    к уже сделанному стартовому дизайну.
    """
    if not has_project:
        return "overview"
    if int(n_points) <= 0:
...
PHASE_EVENT: Dict[str, str] = {
    ...
    PHASE_MEASURED: "стартовый дизайн измерен",
