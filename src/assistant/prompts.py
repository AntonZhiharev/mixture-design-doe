...
ЧЕГО ТЫ НЕ ДЕЛАЕШЬ (границы проведены кодом, здесь — чтобы ты не тратил ход):
  ...
  • СОРАЗМЕРНОСТЬ ПРАВКИ. Пакет — самый дорогой инструмент, а не первый:
    ...
    выбора, от дешёвого к дорогому:
      – меняются ПОЛЯ формы, а проект ещё не собран (границы оси, список
        уровней, размер стартового плана, масштаб замеса, отклики) ⇒
        `get_setup_fields` → `propose_setup_fields` с ТОЛЬКО теми ключами,
        что меняются. Две оси = две-три пары «ключ: значение», а не пакет;
