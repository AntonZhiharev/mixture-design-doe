...
|---|---|---|---|
| Роли, closure/simplex, cap, log, `group_order` | ✅ | ⚠️ только JSON | дерево не проецирует v2 |
| `min_phr`/`max_phr` (CPE≥3, PBNK≤8, `P_max` мела) | ✅ | ❌ не видно | `phr_spec_summary_dataframe`: узел/режим/lo/hi/ref/cap — без `role`/`min_phr`/`max_phr`/`scale` |
| Условные границы §4 (немонотонная `hi_φ(T)`, `lo_φ(T)`) | ✅ `point_report` | ❌ 0 вызовов в UI | golden §4 из интерфейса не сверить |
...
| **P1.3** | iter50 | Видимость спеки: `role`/`min_phr`/`max_phr`/`scale`/`group_order` в сводке + блок «эффективные границы точки» (`point_report`, метки `active`) |
