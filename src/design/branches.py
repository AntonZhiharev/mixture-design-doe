...
# ----------------------------------------------------------------------
# §5/§12 РОЛИ ОТКЛИКОВ — атрибут пары (ветка × отклик), НЕ глобальное свойство
# отклика. Один и тот же отклик в разных ветках может носить разные роли
# (побочный выход: в ветке-A минимизируем как ЦЕЛЬ, в ветке-B учитываем в ЦЕНЕ).
# XOR честности (§5) держится ВНУТРИ ветки и разрешается ПРИОРИТЕТОМ (M2):
#     OPTIMIZED  >  PRICE_INPUT  >  REFERENCE
# — поэтому роль всегда однозначна. Сосуществование «ρ=цель И ρ питает цену»
# РАЗРЕШЕНО: роль ρ становится OPTIMIZED, а двойной счёт убирает атрибуция (Гр-1),
# зануляя σ_ρ-разведочный ценовой канал (см. economic_stop, шаг B). Tag сам по
# себе НЕ спасает — спасает атрибуция, читающая роль.
ROLE_OPTIMIZED = "optimized"      # отклик — ЦЕЛЬ качества (нога d_i в goal ветки)
ROLE_PRICE_INPUT = "price_input"  # отклик питает цену изделия (ρ ценовой ноги)
ROLE_REFERENCE = "reference"      # меряется оракулом, но ни цель, ни цена (справка)
# Порядок = приоритет разрешения XOR (раньше → выше).
ROLE_PRIORITY = (ROLE_OPTIMIZED, ROLE_PRICE_INPUT, ROLE_REFERENCE)
ALL_ROLES = frozenset(ROLE_PRIORITY)
