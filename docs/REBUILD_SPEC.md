...
**OPEN_QUESTIONS (не решено, вынесено):** feasibility-aware acquisition —
`P(измеримо | x)` из паттерна MISSING (классификатор/GP по гейту) как
множитель к `acq`/argmax, чтобы ядро не полагалось на дисциплину «гейт в
цели», а выводило измеримость само; и подсказка/гейт в UI «в цели ветки нет
гейт-отклика при наличии пропусков в базе».
