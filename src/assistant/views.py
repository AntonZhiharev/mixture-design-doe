...
#: Заголовок свёрнутого блока с числами (одна формулировка на док и на ленту —
#: свежий ответ и история не должны выглядеть по-разному).
NUMBERS_TITLE = "📊 Числа и источники"


@dataclass(frozen=True)
class AnswerView:
    """Ответ ассистента, разложенный для показа: текст + числа отдельно (iter70).

    Раздел ``## ЧИСЛА`` формата (iter64) существует ради трассируемости: каждое
    число подписано инструментом, который его вернул. В общем потоке markdown он
    выглядел как продолжение рассуждения, из-за чего измерение и интерпретация
    читались одинаково весомо, а модель вдобавок дублировала значения выше «на
    всякий случай». Здесь числа вынесены в ОТДЕЛЬНОЕ поле — док рисует их
    свёрнутым блоком под ответом.
    """
    #: Текст ответа без раздела чисел (то, что рисуется всегда).
    text: str
    #: Тело раздела ``ЧИСЛА``; пусто — инструменты не вызывались.
    numbers: str = ""

    @property
    def has_numbers(self) -> bool:
        return bool(self.numbers.strip())

    @property
    def numbers_title(self) -> str:
        return NUMBERS_TITLE


def answer_view(text: Any) -> AnswerView:
    """Текст ответа модели → :class:`AnswerView` (числа вырезаны из потока).

    Разбор чистый (:func:`assistant.prompts.pop_section`), поэтому формат
    проверяется тестом, а не глазами. Ответ без раздела чисел проходит насквозь:
    старая переписка и сообщения не по формату показываются как есть.
    """
    from .prompts import pop_section     # локально: prompts тянет реестр tools

    rest, numbers = pop_section(str(text or ""), "ЧИСЛА")
    return AnswerView(text=rest, numbers=numbers)
