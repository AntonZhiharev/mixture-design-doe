...
class PhrSpec:
    ...
    def group_members(self) -> Dict[str, List[str]]:
        """iter76: члены share-групп — ``{родитель: [листья]}`` (публично).

        Нужен слоям выше (`set_preflight_pairs`, пакет проекта), чтобы имя
        УЗЛА-ГРУППЫ (`FILLER`, `SOFT`) можно было развернуть в сумму его
        членов: группа — не координата полной схемы, но её тотал — это ровно
        сумма листьев, то есть валидная ось-сумма preflight-пары.
        """
        return {str(k): list(v) for k, v in self._share_groups.items()}
