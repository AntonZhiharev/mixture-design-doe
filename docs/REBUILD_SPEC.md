...
пропусков в базе»; автоматический вывод гейта из паттерна MISSING (сейчас
гейт объявляет человек).
...
- **После restrict суррогат гейта забывает дыру.** `move_region` →
  ...
  Рабочий приём сейчас — строить множитель **до** сужения (как в каскаде:
  `edge_region` и `gate_feasibility_fn` считаются по суррогату на всей базе).
  Кандидат на решение: суррогат гейта (и только его) учить на всей истории,
  не на активном пуле — гейт описывает физику измеримости, а не область
  интереса. Не сделано: меняет контракт «одна модель на активной базе».
