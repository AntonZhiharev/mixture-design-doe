...
| §3 | $\rho$ как GP-свойство; `price_изд = price_состав / ρ` | общий словарь `runner.surrogates`; функция цены в `src/optimize/desirability.py` |
| §4 | двойной стоп; `stop_reason ∈ {ceil_reached, stagnation, not_economical}` | стоп-слой над `Branch.is_stagnating` + раннер `run_branch_round` |
| §5 | per-property MC-EI с разложением вклада, $x_{next}=\arg\max\text{EI}_{₽}$ | новый модуль VoI (на базе `branch_scores`/`GPExpert.predict.std`) |
| §5.3 | калибровка $\sigma$ (premium-угол) | тест на `GPExpert` (фундамент, ПЕРВЫМ) |
