...
def architect_system_prompt(*, project: str = "", spec_hash: str = "",
    ...
    if not has_runner:
        ...
        ctx.append("  • проект НЕ собран: движка и базы точек нет. Вопросы про "
                   "preflight/прогоны честно оставляй без чисел. Заводить "
                   "проект — твоя работа: собери пакет ПРОЕКТА (состав + "
                   "отклики + процесс-оси) и предложи `propose_project`; "
                   "пакет СПЕКИ сейчас применять НЕКУДА, человек нажмёт "
                   "кнопку и не получит результата.")
    if n_attachments:
