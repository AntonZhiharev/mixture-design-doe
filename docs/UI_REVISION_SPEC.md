...
* Hash `049d0e35…` — от `RECIPE_DICTS` iter35; в тесты нового контракта
  НЕ закладывать (C4). Hash текущей плоской спеки `_phr_spec_pvc.json` —
  `44f0f96a…`. После B1/B2/B4/B5 пересчитать и записать **в момент
  сетапа**, вместе с лотами сырья и anchor-рецептами
  (CAMPAIGN_SPEC_PVC §3).
...
Рекомендуемый порядок: iter45–46 (B1, B2) → iter42 → далее по плану.
После B4 геометрия спеки закрыта окончательно.
