...
def parse_component_prices(text: str,
    ...
    iter85: ПУСТАЯ строка → ``{}`` («цены ещё не заданы»), а не отказ. Цена
    сырья приходит из снабжения позже, чем собирается проект, и гейт на сборке
    заставлял выбирать между задержкой опытов и выдуманными нулями. Пустой
    набор ценовой ноги не даёт вовсе (``set_project_economics`` оставит
    ``component_prices`` пустыми, себестоимость не считается) — это НЕ то же,
    что «все цены = 0», где сырьё объявлено бесплатным. Частичный ввод
    по-прежнему отказ: там нули действительно доопределились бы молча.
    ...
    if vals is None or len(vals) != len(comps):
        raise ValueError(
            ...
            f"(в том же порядке) — либо оставьте поле ПУСТЫМ, если цены пока "
            f"неизвестны: тогда себестоимость просто не считается, пока вы не "
            f"внесёте цены в «📋 Настройки проекта → 💰 Экономика проекта». "
            f"Частичный список недопустим: недостающие цены доопределились бы "
            f"нулём и занизили себестоимость молча.")
    return {c: float(v) for c, v in zip(comps, vals)}


def stale_prices_count(text: str, mixture_names: Sequence[str]) -> Optional[int]:
    """iter85: сколько цен лежит в поле, если это НЕ подходящий набор (иначе ``None``).

    Зачем: значение поля цен переживает смену состава — оно сохраняется в
    ``setup_draft.json`` и возвращается в форму через ``setup_prefill_pending``.
    Черновик ПВХ-проекта несёт ``setup_econ_prices = "0, 0, 0"`` (записано, когда
    в составе стояли дефолтные «A, B, C»), а компонентов в phr-спеке 18 — и
    сборка отказывала «Нужно 18 цен», хотя пользователь этих нулей уже не видел
    и не вводил. Никакой перезапуск приложения это не лечит: значение приходит
    с диска, а Streamlit при существующем ключе игнорирует новый ``value=``.

    Возвращает ЧИСЛО значений в поле, когда их непустой набор не совпадает с
    числом компонентов (⇒ поле устарело, его надо очистить и сказать об этом), и
    ``None``, когда поле пусто, нечисловое или согласовано с составом. Чистая
    функция (без Streamlit) — тестируется напрямую.
    """
    vals = _parse_floats(text)
    if not vals:                      # пусто или нечисловое — не «устарело»
        return None
    return None if len(vals) == len(list(mixture_names)) else len(vals)
...
def seed_consumption_dataframe(runner, Xs, batch_kg: float) -> pd.DataFrame:
    """Аудит 13.08.2026: расход сырья плана ОТДЕЛЬНОЙ таблицей (лист Excel).

    Раньше расход шёл дублирующими колонками «{компонент} (кг)» в основном
    листе (при q≈18 — вторая копия всех компонентов, нечитаемо), а ИТОГА по
    компонентам на весь план не было вовсе — заказать сырьё по файлу было
    нельзя. Здесь: строка = опыт (кг = доля·размер пробы), последняя строка
    «Итого на план» = сумма по каждому компоненту + столбец «Σ (кг)».
    Чистая (без Streamlit)."""
    kg = float(batch_kg)
    if kg <= 0:
        return pd.DataFrame()
...
def seed_design_excel_bytes(runner, Xs, Ys=None, *,
    ...
    ЕДИНЫЙ МАСШТАБ МАССЫ (решение технолога 13.08.2026): если задан
    ``batch_kg``, граммы листа «Навеска» считаются от РАЗМЕРА ПРОБЫ
    (``batch_grams_per_phr(spec, batch_kg)``), а не от переданного
    ``grams_per_phr`` (вес замеса из паспорта) — иначе в одном файле жили два
    разных масштаба массы. ``delta_phr`` (сетка actual) при этом не
    пересчитывается: δ — свойство весов и зафиксированного плана."""
    import io
    df = seed_design_dataframe(runner, Xs, Ys)
    buf = io.BytesIO()
    with pd.ExcelWriter(buf, engine="openpyxl") as xw:
        ...
        if batch_kg is not None and float(batch_kg) > 0:
            ...
            if not cons.empty:
                cons.to_excel(xw, sheet_name="Расход сырья", index=False)
        if spec is not None and delta_phr is not None:
            gpp_eff = grams_per_phr
            if batch_kg is not None and float(batch_kg) > 0:
                try:
                    gpp_eff = batch_grams_per_phr(spec, float(batch_kg))
                except ValueError:
                    gpp_eff = grams_per_phr
            wdf = seed_weighing_dataframe(runner, spec, Xs, float(delta_phr),
                                          grams_per_phr=gpp_eff)
            if not wdf.empty:
...
def batch_sigma_phr(spec: PhrSpec) -> Tuple[float, float]:
    ...
    **iter84:** сама сумма считается ЯДРОМ (:meth:`PhrSpec.sigma_phr_bounds`),
    здесь остаётся только имя, привычное слою UI. Причина переноса: то же
    число независимо считал инструмент ассистента, и там жила ошибка двойного
    счёта тоталов групп — окно технолога и ответ помощника показывали разные
    Σphr. Одна геометрия обязана давать одно число.
    Чистая (без Streamlit)."""
    return spec.sigma_phr_bounds()
...
def render_project_economics(runner) -> None:
    ...
    if not rep["enabled"]:
        ...
        return
    # iter85: «нет ρ» и «нет цен» — РАЗНЫЕ состояния, и лечатся по-разному.
    # ρ — отклик: объявить её на непустой базе нельзя без домера плотности у
    # снятых точек, поэтому это по-прежнему дело пересборки. А цены — факт
    # снабжения: они вносятся ЗДЕСЬ в любой момент, в том числе ПЕРВЫЙ раз.
    # До iter85 панель при пустых ценах отсылала в форму «🆕 Новый проект», а
    # форма без цен не собирала проект — получался замкнутый круг.
    if not rep["rho_property"]:
        st.caption("Экономика включена, но ρ-отклик не объявлен: "
                   "себестоимость не считается. Плотность — измеряемый отклик, "
                   "поэтому объявить её можно только в форме «🆕 Новый проект» "
                   "(на снятых точках её пришлось бы домерять).")
        return
    if not rep["prices"]:
        st.warning(
            "Цены сырья пока НЕ заданы: себестоимость не считается, ветки "
            "рождаются техническими (денежный стоп §4 не применяется). Это не "
            "нули — нуль означал бы «сырьё бесплатно». Введите цены ниже: база "
            "опытов и схема при этом не затрагиваются.")
    cur = rep["currency_unit"] or CURRENCY_UNIT
    mass = rep["mass_unit"] or MASS_UNIT
    st.caption(
    ...
    with st.form("proj_prices_form"):
        st.caption(
            ("Ввод цен ДЕЙСТВУЮЩЕГО проекта (сейчас они не заданы): база опытов"
             if not rep["prices"] else
             "Правка цен ДЕЙСТВУЮЩЕГО проекта: база опытов")
            + f" ({rep['n_points']} точек) и схема не затрагиваются — "
              f"измеренные свойства от цены сырья не зависят.")
        txt = st.text_input(
            f"Цены компонентов {rep['mixture_names']}, {cur}/{mass} "
            "(через запятую)",
            # iter85: цен нет ⇒ поле ПУСТОЕ. Предзаполнять нулями нельзя:
            # «0» означает «сырьё бесплатно», и один клик «Применить» тихо
            # превратил бы «не знаю» в заниженную себестоимость (A0.6).
            ...
            help="В том же порядке, что компоненты. Значение нужно для КАЖДОГО "
                 ...
                 "себестоимость и уводит оптимум к дорогому сырью. Пустое поле "
                 "— «цены не заданы»: себестоимость не считается (уже заданные "
                 "цены при этом снимаются).")
...
def render_setup_form() -> None:
    ...
    if _pending:
        ...
        # setup_draft.json и возвращается ЭТИМ ЖЕ префиллом. Черновик
        # ПВХ-проекта нёс «0, 0, 0» от дефолтных «A, B, C», а компонентов в
        # спеке 18 — сборка отказывала «Нужно 18 цен», хотя этих нулей
        # пользователь не вводил и на экране не видел. Перезапуск приложения не
        # помогал: значение приходит с ДИСКА, а Streamlit при существующем ключе
        # игнорирует новый value=. Поэтому сверяем здесь, ДО инстанцирования
        # виджета, и чистим — но НЕ молча (сообщение у блока экономики, A0.6).
        #
        # ВАЖНО: только для значений ПРЕФИЛЛА. Частичный ввод, сделанный
        # человеком в этом сеансе, чистить нельзя — он обязан приводить к
        # ЯВНОМУ отказу сборки, иначе «95, 200» на трёх компонентах молча
        # превратилось бы в «цены не заданы» (тест поймал именно это).
        _stale_n = stale_prices_count(
            str(_pending.get("setup_econ_prices", "") or ""),
            setup_mixture_names(
                _parse_names(st.session_state.get("setup_mix", "")),
                st.session_state.get("setup_phr_spec_obj")))
    ...
    with st.expander("🆕 Новый проект — настройка области опытов (§17.4)",
        ...
        if econ_on:
            ...
            pc2 = st.columns([3, 2])
            # iter85: имена — из ТОГО ЖЕ источника, что и сборка проекта
            # (setup_mixture_names): в режиме phr-спеки текстовое поле
            # «Компоненты смеси» игнорируется, компоненты берутся из спеки.
            # Раньше подпись и дефолт строились по текстовому полю, а гейт
            # сборки требовал цены по спеке — с 18-компонентной спекой форма
            # просила «3 цены», а сборка отказывала «нужно 18», и ввести
            # правильное число было нельзя.
            econ_comps = setup_mixture_names(mix_live, phr_spec_live)
            econ_prices_txt = pc2[0].text_input(
                f"Цены компонентов {econ_comps}, {econ_cur}/{econ_mass} "
                "(через запятую; пусто — цены пока неизвестны)",
                value="",
                key="setup_econ_prices",
                help="В том же порядке, что компоненты выше. Либо цена для "
                     "КАЖДОГО компонента, либо ПУСТО: частичный список — "
                     "отказ, потому что пропуск, доопределённый нулём, тихо "
                     "занижает себестоимость и уводит оптимум к дорогому "
                     "сырью. Пустое поле — законное «цены ещё не заданы»: "
                     "проект собирается, себестоимость не считается, цены "
                     "вносятся позже в «📋 Настройки проекта → 💰 Экономика "
                     "проекта» БЕЗ пересборки и без потери опытов.")
            ...
            # iter85: пустое поле — законное «цены ещё не заданы». Состояние
            # видимое (A0.6): проект соберётся, но себестоимость считаться не
            # начнёт, и ветки до ввода цен будут техническими. Это НЕ нули:
            # нуль означал бы «сырьё бесплатно» и участвовал бы в расчёте.
            elif _pv is not None and not _pv:
                st.info(
                    "Цены сырья пока не заданы — так можно: проект соберётся, "
                    "но себестоимость считаться не будет, а ветки, созданные "
                    "до ввода цен, останутся техническими. Внесите цены позже "
                    "в «📋 Настройки проекта → 💰 Экономика проекта» — без "
                    "пересборки проекта и без потери измеренных опытов.")
            # iter85: поле было очищено выше, потому что несло цены ПОД ДРУГОЙ
            # состав (сохранённый черновик). Молчать нельзя: пользователь имеет
            # право знать, что его прежний ввод отброшен и почему.
            _stale_was = st.session_state.pop("setup_econ_prices_stale_n", None)
            if _stale_was is not None:
                st.warning(
                    f"Поле цен очищено: в нём лежало {int(_stale_was)} значений "
                    f"под ПРЕЖНИЙ состав, а компонентов сейчас "
                    f"{len(econ_comps)} (значение сохранялось в черновике "
                    f"проекта вместе с формой). Введите цены заново по текущему "
                    f"составу либо оставьте поле пустым и внесите их позже.")
    ...
    if spec_w is not None:
        with st.expander("⚖️ Навеска (phr → граммы): шаг весов, премикс, "
            ...
            step_g = wc[0].number_input(
                "Цена деления весов, г", min_value=0.0,
            ...
            # iter84 (хвост iter83 «а»): поле спрашивает ВЕС ЗАМЕСА, а не
            # «г на 1 phr». Величину «г на 1 phr» в цехе не знают — чтобы её
            # назвать, технолог должен сначала перебрать рецепты и выбрать тот,
            # где ввод компонента максимален (формулировка сессии 12.08.2026:
            # «реально так никто не делает»). Здесь замес относится к КОНКРЕТНОЙ
            # загрузке смесителя и переопределяет паспортный дефолт — поэтому
            # поле осталось, но говорит на языке цеха.
            #
            # Ключ `setup_weigh_gpp` СОХРАНЁН: он присваивается при загрузке
            # проекта (`campaign_state`, не входит в SETUP_UNSETTABLE_KEYS), и
            # переименование потеряло бы значение в сохранённых проектах.
            # Семантика ЗНАЧЕНИЯ сменилась — в нём кг, как и в паспортном поле.
            batch_default = batch_kg_from_grams_per_phr(
                spec_w, getattr(runner, "grams_per_phr", 0.0))
            batch_kg_in = wc[1].number_input(
                "Вес одного замеса рецепта, кг", min_value=0.0,
                value=float(batch_default or 25.0),
                step=0.5, format="%.4f", key="setup_weigh_gpp",
                help="Сколько килограммов весит один замес этой загрузки. "
                     "Масштаб «г на 1 phr» программа считает сама по спеке "
                     "(по верху Σphr — осторожная оценка), из него получается "
                     "δ_phr = цена деления / (г на 1 phr). Дефолт — из паспорта "
                     "кампании (P2.3), если он задан.")
            # Перевод «вес замеса → г/phr» делает тот же хелпер, что и паспорт
            # (iter83): два разных пути к одному числу разошлись бы, и δ в
            # seed-цикле отличалась бы от δ при сборке проекта.
            gpp: float = 0.0
            if float(batch_kg_in) > 0:
                try:
                    gpp = batch_grams_per_phr(spec_w, float(batch_kg_in))
                except ValueError as exc:
                    st.error(str(exc))
                    gpp = 0.0
                else:
                    s_lo, s_hi = batch_sigma_phr(spec_w)
                    st.caption(
                        f"Σphr спеки {s_lo:.4g}…{s_hi:.4g} ⇒ масштаб "
                        f"{gpp:.4g} г на 1 phr (по верху Σphr — осторожная "
                        f"оценка разрешения).")
            if float(step_g) > 0 and float(gpp) > 0:
                ...
                st.caption("Пересчёт в граммы выключен (цена деления весов или "
                           "вес замеса = 0): план фиксируется как расчётный "
                           "(nominal).")
