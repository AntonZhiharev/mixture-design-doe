...
* Hash `049d0e35…` — от `RECIPE_DICTS` iter35; в тесты нового контракта
  ...
  текущий hash — `7769ce2152788efb…` (изменится после B4/B5). Финальный
  хеш пересчитать и записать **в момент сетапа**, вместе с лотами сырья
...
6. **`scale`**: схема принимает `linear`/`log` (`log` — только absolute,
   ...
   (`sample_z`/`clip_z`/`z_bounds`) до iter47 (B5) отвергают log-оси явной
   ошибкой — не искажают меру молча.
