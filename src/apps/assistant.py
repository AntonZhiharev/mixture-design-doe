...
from src.apps.battle_preset import battle_step6_description
...
def build_context(runner, extra: Optional[Dict[str, Any]] = None,
    ...
    ctx: Dict[str, Any] = {
        ...
        # статическая карта интерфейса — чтобы ассистент отвечал на вопросы про
        # кнопки/вкладки/навигацию («где удалить проект?»), а не разводил руками
        "ui_guide": ui_guide(),
        # развёрнутое описание пресета кнопки «🧪 Заполнить тестовыми данными»
        # (компоненты, цены, свойства, ИЗВЕСТНАЯ истина лаборатории) — чтобы
        # ассистент мог рассказать пользователю, что это за тестовые данные
        "test_data_preset": battle_step6_description(),
    }
...
def form_context(cfg) -> Dict[str, Any]:
    """Срез ЖИВОЙ формы сайдбара (то, что пользователь ввёл в интерфейсе).

    В отличие от :func:`_config_brief` (краткий конфиг запущенного runner), это
    ПОЛНЫЙ снимок текущих настроек формы — включая ограничения состава, режим
    массовых частей, стоимость компонентов, размер пробы и разбиение на блоки.
    Может отличаться от состояния запущенного проекта (пользователь ещё не
    нажал «Создать/сбросить»). Возвращает ``{"form": {...}}`` для вложения в
    ``build_context(..., extra=...)`` → попадёт в контекст как ``ui.form``.
    """
    if cfg is None:
        return {}
    ...
    composition = {
        "mode": getattr(cfg, "comp_mode", None),
        "lower": _list(getattr(cfg, "lower", None)),
        "upper": _list(getattr(cfg, "upper", None)),
        "base_index": getattr(cfg, "base_index", None),
        "parts_min": _list(getattr(cfg, "parts_min", None)),
        "parts_max": _list(getattr(cfg, "parts_max", None)),
    }
    form = {
        "name": getattr(cfg, "name", None),
        "q": getattr(cfg, "q", None),
        "names": _list(getattr(cfg, "names", None)),
        "property_names": _list(getattr(cfg, "property_names", None)),
        "model": getattr(cfg, "model", None),
        "noise_sd": getattr(cfg, "noise_sd", None),
        "seed": getattr(cfg, "seed", None),
        "n_runs_factor": getattr(cfg, "n_runs_factor", None),
        "n_restarts": getattr(cfg, "n_restarts", None),
        "n_blocks": getattr(cfg, "n_blocks", None),
        "composition": composition,
        "cost_unit": getattr(cfg, "cost_unit", None),
        "cost_coeffs": _list(getattr(cfg, "cost_coeffs", None)),
        "batch_size": getattr(cfg, "batch_size", None),
        "batch_unit": getattr(cfg, "batch_unit", None),
    }
    return {"form": form}


# ----------------------------------------------------------------------
# Карта интерфейса (статическая) — чтобы ассистент мог подсказывать «где нажать»
# ----------------------------------------------------------------------
# Это КУРИРУЕМОЕ описание раскладки Streamlit-приложения (а не динамика). Нужно,
# чтобы ассистент отвечал на вопросы вида «где кнопка удаления проекта?». При
# изменении UI (streamlit_app.py) этот гид надо обновлять синхронно.
def ui_guide() -> Dict[str, Any]:
    """Статическая карта элементов интерфейса (сайдбар + вкладки + кнопки)."""
    return {
        "_note": "Карта интерфейса приложения (где что нажимать). Это описание "
                 "раскладки UI, а не динамическое состояние — опирайся на него, "
                 "когда пользователь спрашивает про кнопки/вкладки/навигацию.",
        "sidebar": {
            "📁 Проект": [
                "selectbox «Открыть сохранённый проект» + кнопка «📂 Загрузить "
                "проект» — открыть ранее сохранённый проект целиком.",
                "Раскрывающийся блок «🗑 Удалить проект (admin)»: выбрать проект, "
                "вписать его имя для подтверждения, ввести пароль администратора "
                "(переменная окружения DOE_ADMIN_PASSWORD), затем кнопка "
                "«🗑 Удалить навсегда». Это барьер от случайного удаления.",
                "Кнопка «💾 Сохранить проект» — сохранить весь проект на диск.",
            ],
            "⚙️ Конфигурация проекта": [
                "Имя проекта, число компонентов q, модель Шеффе, шум σ, seed, "
                "n_runs=factor·p, GP/опт. restarts, число блоков (партий/дней).",
                "Раскрывающиеся блоки: «🏷️ Имена компонентов», «🎯 Целевые "
                "свойства (отклики)», «📐 Ограничения состава» (доли 0…1 или "
                "массовые части, база=100), «💰 Стоимость компонентов».",
            ],
            "📦 Размер пробы": [
                "Размер партии/пробы + единица — доли пересчитываются в "
                "количество сырья в таблицах планов и в рецепте.",
            ],
            "🔧 Создать / сбросить проект": [
                "Кнопка инициализации/сброса проекта по текущей конфигурации.",
            ],
            "🧪 Заполнить тестовыми данными": [
                "Кнопка-пресет: одним кликом заполняет форму эталонной "
                "конфигурацией (4 компонента {A,B,C,D}, реальные цены, 5 "
                "свойств) с ИЗВЕСТНОЙ детерминированной истиной лаборатории. "
                "Удобно для демонстрации и проверки: можно сразу пройти M1…M8 и "
                "сверить рецепт с оптимумом во вкладке «🎯 Benchmark». Подробное "
                "описание набора — в блоке `test_data_preset`.",
            ],

            "💾 Чекпоинты": [
                "selectbox чекпоинта + «↩ Загрузить состояние» — откатиться к "
                "сохранённому состоянию стадии.",
            ],
        },
        "tabs": {
            "M1…M8": "Пошаговый прогон стадий конвейера. На каждой вкладке кнопка "
                     "«▶ Выполнить {стадия}» (или «♻️ Пересчитать»). На M2 — "
                     "редактор откликов: «💾 Сохранить отклики» и «🧪 Заполнить "
                     "тестовыми (симулятор)».",
            "🌿 Ветки": "Создание веток (цель+бюджет), портфельный раунд "
                       "(арбитр слотов), диагностика модели и экономика стоп/"
                       "границ.",
            "🧬 Кампания": "Кампания с per-branch ролями откликов поверх "
                          "отдельного MixtureProcessRunner.",
            "🎯 Benchmark": "Кнопка «▶ Сравнить с истиной» — сравнение рецепта "
                           "pipeline с известным оптимумом полигона.",
            "💬 Ассистент": "Этот чат: настройки ключа/модели, кнопка "
                           "«🔄 Опубликовать снапшот для Cline», блок «👁️ Что "
                           "видит ассистент», поле ввода вопроса.",
        },
    }


# ----------------------------------------------------------------------
# §17.6 — campaign-native контекст ассистента (salvage без PipelineRunner)
# ----------------------------------------------------------------------
# Перенос ИИ-помощника на единый движок кампании (§17.6, «сначала перенести
# нужный интерфейс, потом сносить старое»): контекст строится ПРЯМО из сводки
# кампании (campaign.campaign_overview), без PipelineRunner/стадий M1…M8. Пока
# используется как параллельный путь; финальный снос M1…M8 UI переведёт главный
# поток на него целиком.
def campaign_ui_guide() -> Dict[str, Any]:
    """Карта ЭКРАНА проекта (iter72/iter74): три зоны + закладки рабочей области.

    Порядок работы §17: настройка области → стартовый план → ветки → рабочий
    стол → изменение схемы. ОДИН движок физики (MixtureProcessRunner), ветка —
    контейнер намерения, отклики вносятся вручную (§17.2/§17.4).

    iter74: карта описывает ФАКТИЧЕСКУЮ раскладку (сайдбара нет, шаги разложены
    по закладкам) и НАЗВАНИЯ КНОПОК так, как они подписаны в интерфейсе. Иначе
    ассистент отправляет человека к кнопкам, которых на экране не существует.
    """
    return {
        "_note": "Карта экрана проекта (§17, раскладка iter72/iter74). Один "
                 "движок физики, отклики вносятся вручную. Отвечая «где "
                 "нажать», называй зону, закладку и подпись кнопки ровно так, "
                 "как здесь.",
        "layout": {
            "левая зона": "переписка с помощником («💬 Помощник по проекту») + "
                "панели утверждения его предложений: «🏗 Предложенные проекты "
                "(пакетом)», «🧬 Предложенные спеки (пакетом)», «🧩 "
                "Предложенные патчи спеки». Применяет и отклоняет только "
                "человек кнопками.",
            "центр": "рабочая область на ЗАКЛАДКАХ (по одному шагу за раз): "
                "«🌱 Старт», «📚 База опытов», «🌿 Ветки», «📊 Анализ», "
                "«🧬 Схема», «🤖 Обзор». Недоступная закладка видна, но "
                "выключена с причиной. Второй ряд закладок на «🌿 Ветках» — "
                "сами ветки проекта.",
            "правая зона": "инфо-панель проекта: «📎 Вложения», «🖼 Файлы "
                "расчётов помощника», «📝 Предложенные записи в журналы» "
                "(формулировки помощника: поля можно ПОПРАВИТЬ, кнопки "
                "«✅ Зафиксировать решение» / «✅ Зафиксировать факт» / "
                "«⛔ Отклонить запись»), «📚 Журнал решений» (история принятого "
                "и отклонённого + кнопка «✍️ Записать решение в журнал»), "
                "«🏭 Факты производства (L1)» (знание цеха, приоритетнее "
                "литературы, + кнопка «✍️ Записать факт в журнал»), "
                "«📌 Состояние переписки с помощником».",
            "сайдбара нет": "боковой панели в приложении НЕТ — вход в проект "
                "(создать/сохранить/загрузить) живёт на закладке «🌱 Старт».",
        },
        "flow": {
            "1. Настройка области (§17.4)": "закладка «🌱 Старт»: панель "
                "«📁 Проект» (сохранить/загрузить/удалить) и форма «🆕 Новый "
                "проект — настройка области опытов» (компоненты смеси, "
                "процесс-параметры с границами, отклики, условия прогона) "
                "→ кнопка «🏗 Построить проект».",
            "2. Стартовый план опытов (§17.4)": "закладка «🌱 Старт», блок "
                "«🌱 Стартовый план опытов»: «📐 Предложить стартовый план» → "
                "внести измеренные отклики по всем свойствам → «💾 Зафиксировать "
                "стартовый план и отклики». Кнопка «🧪 Заполнить тестовыми "
                "значениями (симулятор)» подставляет расчётные значения вместо "
                "лаборатории — только для проверки хода работы.",
            "3. Ветки (§17.5)": "закладка «🌿 Ветки», форма «➕ Создать ветку "
                "вручную»: несколько целей (отклик/вид/диапазон/вес), "
                "необязательная ценовая нога ρ + цены состава. Дочерняя ветка — "
                "блок «🌱 Создать дочернюю ветку».",
            "4. Рабочий стол ветки (§17.2)": "закладка «🌿 Ветки» → блок "
                "«🛠 Рабочий стол ветки»: «📐 Предложить точки (база не "
                "меняется)» → внести отклики → «💾 Добавить измеренные точки в "
                "базу» → решение об остановке (§4).",
            "5. База опытов": "закладка «📚 База опытов»: выгрузка в Excel, "
                "«✏️ Исправить измеренные отклики», «📈 Условия прогона».",
            "6. Анализ": "закладка «📊 Анализ»: «📊 Рассчитать матрицу влияний "
                "(компонент × свойство)» и «🔬 Разобрать свойство».",
            "7. Изменение схемы (§16.2)": "закладка «🧬 Схема»: раскрыть "
                "ось/компонент, ввести отклик, подвинуть границы (перенос "
                "старых точек — по явной политике).",
        },
    }


def build_campaign_context(overview: Dict[str, Any], *,
                           extra: Optional[Dict[str, Any]] = None
                           ) -> Dict[str, Any]:
    """§17.6: JSON-контекст ассистента ПРЯМО из сводки кампании (без PipelineRunner).

    ``overview`` — выхлоп ``campaign.campaign_overview`` (property_names, n_points,
    origin_counts, per-branch роли/статус/d_best/покрытие, денежный канал ρ). Это
    campaign-native замена :func:`build_context`: ОДИН движок физики (§5/§12),
    стадий M1…M8 нет. Возвращает JSON-сериализуемый снимок с картой UI кампании.
    """
    ctx: Dict[str, Any] = {
        "generated_at": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "mode": "campaign",
        "property_names": list(overview.get("property_names", []) or []),
        "n_points": int(overview.get("n_points", 0) or 0),
        "origin_counts": dict(overview.get("origin_counts", {}) or {}),
        "branches": overview.get("branches", []),
        "coverage_fraction_threshold": overview.get("coverage_fraction_threshold"),
        "ui_guide": campaign_ui_guide(),
    }
    if extra:
        ctx["ui"] = extra
    return _jsonable(ctx)
...
def write_live_snapshot(runner, root: Optional[str] = None,
                        ...
                        form: Optional[Dict[str, Any]] = None) -> Dict[str, str]:
    ...
    ``form`` (опц.) — срез живой формы сайдбара (:func:`form_context`); пишется
    в ``meta.form``, чтобы Cline видел текущие настройки интерфейса.
    ...
    meta = {
        ...
        # развёрнутое описание набора кнопки «🧪 Заполнить тестовыми данными» —
        # чтобы Cline через MCP (run_overview.meta) видел те же сведения, что и
        # встроенный ассистент в контексте build_context (test_data_preset).
        "test_data_preset": battle_step6_description(),
    }
    if form:
        meta["form"] = form.get("form", form) if isinstance(form, dict) else form
...
def system_prompt() -> str:
    return (
        ...
        "конфиг, ветки). Опирайся на эти ЧИСЛА, а не на догадки. Если есть блок "
        "`ui.form` — это ТЕКУЩИЕ настройки формы сайдбара (живой ввод "
        "пользователя в интерфейсе: q, имена компонентов, целевые свойства, "
        "ограничения состава/массовые части, стоимость, размер пробы, блоки). "
        "Они отражают, что пользователь видит и настраивает прямо сейчас, и "
        "могут отличаться от запущенного проекта (если он ещё не пересоздан). "
        "Используй `ui.form`, чтобы понимать намерения и текущий ввод. В блоке "
        "`ui_guide` — КАРТА интерфейса (сайдбар, вкладки, кнопки и их назначение). "
        "Когда пользователь спрашивает «где это нажать / как это сделать в UI» "
        "(например, как удалить проект) — отвечай по `ui_guide`, а не «я не вижу "
        "интерфейс».\n\n"
        "В блоке `test_data_preset` — развёрнутое описание кнопки «🧪 Заполнить "
        "тестовыми данными»: компоненты с ценами, целевые свойства, модель/seed/"
        "шум и ИЗВЕСТНАЯ детерминированная истина лаборатории (коэффициенты "
        "термов Шеффе по каждому свойству в `known_truth`). Когда пользователь "
        "спрашивает, что это за кнопка/тестовые данные, что они моделируют или "
        "как ими пользоваться — рассказывай по `test_data_preset` (это эталонный "
        "демо-набор для прогона M1…M8 и сверки с Benchmark), не выдумывая чисел.\n\n"
...
def campaign_system_prompt() -> str:
    ...
    главный поток — настройка области (mixture+процесс+отклики) → стартовый
    план опытов → ручные мультицелевые ветки → рабочий стол (ручной ввод
    откликов) → изменение схемы. Стадий M1…M8 и PipelineRunner больше нет — не
    упоминать их пользователю.

    iter74: промпт согласован с ФАКТИЧЕСКИМ экраном (три зоны + закладки, без
    сайдбара) и с языком интерфейса: подписи кнопок операционные («Добавить
    измеренные точки в базу», «Отменить последнюю настройку»), внутренние
    словечки (seed/долив/undo/spawn/стейдж/выхлоп) в ответ пользователю не
    выносятся.
    """
    return (
        "Ты — встроенный помощник инженерного приложения для планирования "
        "экспериментов со смесями (mixture DOE). Единственный рабочий порядок — "
        "ПРОЕКТ (§17): 1) настройка области опытов (компоненты смеси Σ=1, "
        "процесс-параметры, отклики); 2) стартовый план опытов с внесением "
        "измеренных откликов пользователем; 3) ручное создание веток — каждая "
        "ветка это НАБОР целей (несколько целей: min/max/target, диапазон, вес) "
        "+ необязательная модель себестоимости изделия (ρ-отклик × цена "
        "состава); 4) рабочий стол ветки: предложить точки → внести измеренные "
        "отклики → добавить точки в общую базу → решение об остановке (§4: "
        "техническое И экономическое); 5) изменение схемы в любой момент "
        "(раскрыть ось/компонент/отклик, подвинуть границы) с явной политикой "
        "переноса старых точек. Единая модель физики на проект (общие "
        "GP-модели на каждое свойство); ветка — лишь контейнер намерения, своей "
        "модели не имеет.\n\n"
        "Тебе передаётся JSON с актуальным состоянием проекта: property_names, "
        "n_points (размер общей базы), origin_counts (откуда точки — стартовый "
        "план / ветки), branches (id/статус/бюджет/d_best/роли/денежный канал ρ "
        "по каждой ветке). Опирайся на эти ЧИСЛА, а не на догадки. В блоке "
        "`ui_guide` — КАРТА ЭКРАНА: три зоны (слева переписка с тобой и панели "
        "утверждения твоих предложений, в центре рабочая область на ЗАКЛАДКАХ, "
        "справа инфо-панель) и подписи кнопок. Бокового меню (сайдбара) в "
        "приложении НЕТ: вход в проект — закладка «🌱 Старт». Когда пользователь "
        "спрашивает «что нажать дальше» — отвечай по `ui_guide`, называя "
        "закладку и подпись кнопки ДОСЛОВНО; кнопок, которых там нет, не "
        "придумывай.\n\n"
        ...
        "(настройка области §17.4 → стартовый план §17.4 → ветки §17.5 → "
        "рабочий стол §17.2);\n"
        "• говори ЯЗЫКОМ ИНТЕРФЕЙСА, а не внутренними именами кода: «стартовый "
        "план опытов» (не seed), «добавить точки в базу» (не долить/commit), "
        "«отменить последнюю настройку» (не undo), «дочерняя ветка» (не spawn), "
        "«предложение ждёт применения» (не стейдж), «файлы расчётов» (не выхлоп "
        "песочницы), «проверка плана» (не preflight), «модели свойств» (не "
        "суррогаты). Внутреннее имя допустимо в скобках, если пользователь сам "
        "им пользуется;\n"
        "• не выдумывай метрики, которых нет в контексте;\n"
