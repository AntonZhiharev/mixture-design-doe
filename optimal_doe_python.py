...
Includes D-optimal and I-optimal designs
"""
...
if __name__ == "__main__":
    print("=== Optimal DOE Generator Demo ===\n")
    
    # Example 1: 2-factor D-optimal design
    print("1. Two-factor D-optimal design:")
    n_factors = 2
    factor_ranges = [(-2, 2), (-1, 3)]  # Custom ranges for factors
    
    doe = OptimalDOE(n_factors, factor_ranges)
    
    # Generate D-optimal design
    d_optimal_design = doe.generate_d_optimal(n_runs=12, model_order=2, random_seed=42)
    d_results = doe.evaluate_design(d_optimal_design, model_order=2)
    
    print(f"D-optimal design ({d_results['n_runs']} runs):")
    print(f"D-efficiency: {d_results['d_efficiency']:.4f}")
    print(f"I-efficiency: {d_results['i_efficiency']:.4f}")
    print("\nDesign points:")
    design_df = pd.DataFrame(d_optimal_design, columns=['Factor_1', 'Factor_2'])
    print(design_df.round(3))
    
    # Generate I-optimal design
    print("\n2. Two-factor I-optimal design:")
    i_optimal_design = doe.generate_i_optimal(n_runs=12, model_order=2, random_seed=42)
    i_results = doe.evaluate_design(i_optimal_design, model_order=2)
    
    print(f"I-optimal design ({i_results['n_runs']} runs):")
    print(f"D-efficiency: {i_results['d_efficiency']:.4f}")
    print(f"I-efficiency: {i_results['i_efficiency']:.4f}")
    print("\nDesign points:")
    design_df = pd.DataFrame(i_optimal_design, columns=['Factor_1', 'Factor_2'])
    print(design_df.round(3))
    
    # Example 2: Multiple factors
    print("\n3. Three-factor D-optimal design:")
    n_factors = 3
    factor_ranges = [(-1, 1), (-1, 1), (-1, 1)]
    
    doe_3f = OptimalDOE(n_factors, factor_ranges)
    d_optimal_3f = doe_3f.generate_d_optimal(n_runs=20, model_order=2, random_seed=42)
    results_3f = doe_3f.evaluate_design(d_optimal_3f, model_order=2)
    
    print(f"3-factor D-optimal design ({results_3f['n_runs']} runs):")
    print(f"D-efficiency: {results_3f['d_efficiency']:.4f}")
    print(f"I-efficiency: {results_3f['i_efficiency']:.4f}")
    
    # Example 3: Multiple response analysis
    print("\n4. Multiple response analysis:")
    
    # Define example response functions
    def response1(X):
        # Linear response: y = 2*x1 + 3*x2 + noise
        return 2*X[:, 0] + 3*X[:, 1] + np.random.normal(0, 0.1, X.shape[0])
    
    def response2(X):
        # Quadratic response: y = x1^2 + x2^2 + x1*x2 + noise
        return X[:, 0]**2 + X[:, 1]**2 + X[:, 0]*X[:, 1] + np.random.normal(0, 0.1, X.shape[0])
    
    def response3(X):
        # Interaction response: y = x1*x2 + 0.5*x1 + noise
        return X[:, 0]*X[:, 1] + 0.5*X[:, 0] + np.random.normal(0, 0.1, X.shape[0])
    
    # Compare designs for multiple responses
    designs = [d_optimal_design, i_optimal_design]
    design_names = ['D-optimal', 'I-optimal']
    response_functions = [response1, response2, response3]
    ...
    # Plot 2D designs if matplotlib is available
    try:
        print("\n5. Plotting 2D designs...")
        doe.plot_design_2d(d_optimal_design, "D-optimal Design")
        doe.plot_design_2d(i_optimal_design, "I-optimal Design")
    except:
        print("Plotting requires matplotlib display capability")
