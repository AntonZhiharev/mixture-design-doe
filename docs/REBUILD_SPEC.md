...
> Жизненный цикл кампании (динамические параметры, мультицелевые ветки, добор
> точек и замкнутый цикл) вынесен в §16.2–§16.6 ниже — это исправление
> принципиальных недочётов §16 (см. §16.2.0).

---

## §16.2–§16.6 — Жизненный цикл кампании (исправление недочётов §16)

> Этот блок дописан, чтобы закрыть четыре принципиальных пробела исходного §16
> (он описывал только РОЛИ откликов и атрибуцию цены, но не ЖИЗНЕННЫЙ ЦИКЛ
> кампании). Канон тот же (`.clinerules`, §5/§12): ОДНА модель физики на проект,
> ветка — контейнер намерения; сначала логика + unit-тест, потом UI. Статус всего
> блока — **E+ / НЕ реализовано** (карта и гейты — §16.6); реализация — отдельная
> сессия.

### §16.2.0 Диагностика недочётов (что было не так)

Зафиксировано по факту кода (сверено с ядром и UI на момент написания):

1. **«С самого начала нельзя добавлять параметры».** Ядро это УЖЕ умеет
   (`MixtureProcessRunner.begin_phase` / `augment_phase_schema` /
   `augment_phase_mixture` / `augment_phase_atomic`, `evolve_schema(add_process=…,
   add_mixture=…, add_responses=…)` + `migrate_point`), и весь battle-тест
   (`test_iteration13_battle.py`) построен ВОКРУГ поэтапного раскрытия
   переменных. Но §16 НЕ описывал добавление параметров как штатную операцию
   кампании — поэтому в ТЗ/UI этого «не видно». → закрывается §16.2.
2. **«Одна ветка — одно целевое значение».** `Branch.goal` КОНСТРУКТИВНО
   мультицелевой (`Dict[str, DesirabilitySpec]`), battle-ветки несут по 3 цели с
   разными диапазонами и весами. Но `streamlit_app.render_branches` создаёт ветку
   как `add_branch(name, {prop: spec})` — РОВНО одно свойство. Ограничение в UI,
   не в ядре. → закрывается §16.3.
3. **«Зайдя в ветку, негде доливать точки».** `run_branch_round` есть в ядре и в
   `CampaignController`, но «рабочего стола ветки» (предложить точки → внести
   измеренные Y → переобучить → пересчитать x*) в основном приложении нет. →
   закрывается §16.4.
4. **«Нет понятного цикла».** Сквозной контур «новые параметры/цели → миграция →
   предложение точек → результаты эксперимента → переобучение → повтор» в ТЗ
   отсутствовал. → закрывается §16.5.

Принцип исправления: ничего не «достраиваем» в ядре сверх нужного — в основном это
**вывод уже существующих возможностей ядра в явный контракт ТЗ и UI**. Где ядру
нужна тонкая обёртка (единый фасад операций кампании) — это помечено в §16.6.

### §16.2 Динамическая схема как штатная операция кампании

**Тезис.** Добавление компонента смеси / процесс-переменной / отклика и
смещение границ (relax/restrict) — НЕ разовый battle-сценарий, а штатная
операция живой кампании (ветка/проект — живой объект, ср. §2.1). ТЗ обязывает
дать её пользователю с самого начала, а не «зашитой» в тест.

**Операции (фасад над `evolve_schema` + `augment_phase_*`):**

| Операция | Семантика | Базис в ядре |
|---|---|---|
| `add_mixture_component(name, lo, hi, migration)` | APPEND компонента в КОНЕЦ симплекса; Σ переопределяется (A+B=1 → A+B+C=1) | `evolve_schema(add_mixture=…)` / `augment_phase_mixture` |
| `add_process_var(name, lo, hi, migration)` | APPEND process-переменной (вне симплекса, A0.4) | `evolve_schema(add_process=…)` / `augment_phase_schema` |
| `add_response(spec)` | новый отклик; у старых точек `Y[new]=MISSING` | `evolve_schema(add_responses=…)` |
| `relax_bounds` / `restrict_bounds` | смена границ области интереса (не append) | `move_region` / `evolve_schema(relax_bounds=…)` |

**Инварианты (binding):**
- **A0.6 — миграция НЕ молча.** Добавление ПЕРЕМЕННОЙ требует ЯВНОЙ политики для
  старых точек: `known-constant(v)` (опыты шли при известном фикс. значении) /
  `unknown` (точка не годна для членов с этой переменной) / `recompute(fn)`.
  Никаких молчаливых 0/средних (`schema_evolution`). Новый ОТКЛИК ⇒ `MISSING` у
  старых точек (суррогат учится только на измеренных).
- **И-1 — общая база не урезается.** Миграция создаёт НОВУЮ версию точек поверх
  истории; старые версии сосуществуют (`SchemaHistory`). Точка либо мигрирует,
  либо честно ВЫПАДАЕТ (`select_fixed_rows` → skipped), но из истории не
  удаляется.
- **Версионирование.** Добавление переменной/отклика ⇒ bump версии схемы
  (`version+1`, `change_log`). Смена ТОЛЬКО границ (relax/restrict) — это
  region-move БЕЗ bump (§15.2.4): другая ось diff'а (`within_new_bounds`).
- **Атомарность фазы (§15.1.5).** Если одним шагом меняются И схема, И область
  (например +P и раскрытие C) — это ОДНА атомарная фаза с одним bump и
  co-record причин.

**Гейт-тест (§16.6).** Кампания-уровневый тест: старт с {A,B} → `add_process_var("T")`
→ `add_mixture_component("C")` с явной миграцией → база не урезана, версия
выросла, мигрированные точки в области, непокрытые — честно skipped.

**`add_response` — ОДНА операция из двух половин (iter99, 15.09.2026).**
До iter99 фасад/UI/карта маршрутов ассистента заявляли «новый отклик; у старых
точек `Y=MISSING`», но выполнялась только половина: `evolve_schema` поднимал
версию, а `runner.property_names`/`prop_index` задавались один раз из оракула
и не росли — `commit_*` отвергал строку Y с лишним столбцом, ветка на новый
отклик не создавалась, столбца не было ни в Y точек, ни в листе «Отклики».
Теперь `CampaignController.add_response` СНАЧАЛА зовёт
`runner.declare_response(name, reason)` (протокол `oracle.declare_response`:
ручной оракул расширяет набор свойств; синтетическая истина с фиксированной
физикой — явный `ValueError`, схема остаётся нетронутой), и только затем bump
схемы. У каждой снятой точки `Y[new]=MISSING` **с причиной** в
`origin_tag["missing_reasons"]` («введён после проведения опыта» или текст
пользователя); суррогат рождается, когда накопятся измерения. Момент ввода
не ограничен стадией — единственная цена: у истории по новому отклику нет
данных. Гейт-тест: `test_iteration19_campaign_schema.py` (обе половины) +
`test_iteration99_torn_domain_battle.py` (гейт-отклик вводится после скрининга).

### §16.2.1 Рваная область определения откликов: канон «гейт в цели ветки»

**Постановка (аудит 15.09.2026).** На лабораторном экструдере большинство
рецептов даёт негодный образец; зависимые отклики (оптика) с него не
снимаются — из 29 откликов на 70 точках полный набор есть лишь в 10. Область
определения функции РВАНАЯ: измерима меньшая часть симплекса × куба.

**Что ядро держит по построению (iter98 + iter99, проверено battle-тестом
`test_iteration99_torn_domain_battle.py` на `TornLab` — гейт `surface` всегда
измерим, `gloss`/`dry_time` = MISSING при `surface < 4`, измеримо ~39 %):**
- скрининг с ~60 % пропусков принимается целиком; каждый пропуск с причиной;
  суррогат каждого свойства учится на СВОИХ измеренных точках;
- суррогат гейта после скрининга предсказывает измеримость точки (accuracy
  1.00 на 2000 пробах) — скрининг реально даёт карту «границ доступного»;
- `move_region` (restrict в измеримую зону / relax назад), `augment_phase_*`
  (новый компонент), save/load — `MISSING` и причины переезжают дословно,
  история не урезается (И-1);
- точка с непроверенной целью рекордом ветки не становится
  (`measured_desirability`).

**Что ядро НЕ делает и чем это закрывается (binding-канон):** acquisition
(`branch_scores`: `acq = (1−e)·d̂ + e·σ̄`) и `optimize_xbest` про измеримость
не знают. GP gated-отклика обучен только на годных образцах и экстраполирует
в дыру, а σ там максимальна — и exploit, и explore тянут в неизмеримую зону.
Замер: ветка **без** гейта в цели сжигает 20 из 20 предложений в дыру,
`d_best = 0`; та же ветка **с** гейтом `hard_threshold_spec(surface ≥ 4)` —
1 промах из 20, `d_best = 0.878` при аналитическом оптимуме 0.877.
Поэтому:

> **Гейт-отклик (условие получения образца) ОБЯЗАН входить в цель каждой
> ветки** — как `hard_threshold_spec(порог, шум)` (ограничение на среднее)
> или `ChanceConstraint` (на хвост). Скрининг определяет гейт и его порог;
> ветки наследуют его как первую ногу цели. Если гейт не был объявлен
> откликом при сборке — вводится живой операцией `add_response` и домеряется
> на новых точках (старым — MISSING с причиной).

**Уточнение эталона (iter100).** `branch_optimum` уточнял скан одним
SLSQP-стартом и садился в локальный бассейн на ~1e-3 ниже глобального —
измеренный `d_best` (0.878) превышал «потолок» (0.877) при нулевом шуме, что
физически невозможно. Теперь мультистарт из top-K точек скана (`n_starts=5`,
как в `masked_branch_optimum`): эталон — строгая верхняя граница.

#### §16.2.1.1 Обрыв: экспоненциальный рост к кромке и «чёрная дыра» explore (iter100)

**Постановка.** Полезный отклик растёт экспоненциально при приближении к
границе измеримости и за ней обрывается (образца нет ⇒ MISSING). Оптимум
цели лежит РОВНО на кромке. Вопрос: обтесает ли ядро зону роста и не станет
ли кромка «чёрной дырой» для GP (за ней μ↑ и σ↑ одновременно — суррогат
экстраполирует рост, а измерять там нечего).

**Мир.** `CliffTruth` = 3-комп battle-истина + гейт `surface` +
неполиномиальный `yield = exp(6.5 − surface) − 1` при `surface ≥ 4`, иначе 0
(`build_truth_3comp_cliff`); лаборатория `TornLab(gated=gloss,dry_time,yield)`.
Аналитический оптимум `{yield max, strength max, surface ≥ 4}`:
`d_opt = 0.9409`, `surface = 4.000`, `yield = y_cliff = 11.18` — на кромке.

**Замер (`test_iteration100_cliff_black_hole.py`, seed 60 точек, 6 раундов × 4):**

| сценарий | промахи по раундам | глубина промаха (4 − surface) | d_best |
|---|---|---|---|
| гейт в цели, explore 0.3 | 0–2 из 4 | ≤ 0.03 (кромка, не дыра) | 0.9400 |
| гейт в цели, explore 0.6, без учёта измеримости | **3 из 4 каждый раунд** | **2.4–2.9** (дно дыры) | 0.9264 |
| то же + `set_branch_gate` (explore × P(измеримо\|x)) | 0–1 из 4 | 0.00 | 0.9398 |
| без гейта в цели + множитель | 4 из 4 | 0.45–0.86 | 0.884 |

Выводы: (1) при умеренном explore ядро подкрадывается к обрыву само —
точки ложатся на кромку с обеих сторон (|surface−4| < 0.01), промахи
мелкие, `d_best → d_opt`; (2) «чёрная дыра» реальна и сидит в
**explore-члене**: нормированная σ̄ максимальна за кромкой, и при
explore ≥ 0.6 все explore-слоты уходят на дно дыры; exploit-слот (argmax
с гейтом в цели) в дыру не идёт; (3) лечение — множитель измеримости.

**Реализация (feasibility-aware explore, закрывает OPEN_QUESTION):**
`design.branches.gate_feasibility(surrogate, thr, dir)` → `P(измеримо|x) =
Φ((μ_gate − thr)/σ_gate)` из ОБЩЕГО суррогата гейта;
`branch_scores(..., feasibility=)` умножает на него ТОЛЬКО explore-член
(`d̂` и `σ̄` не меняются, `feasibility=None` — бит-в-бит прежнее);
`runner.set_branch_gate(branch, response, threshold, direction)` /
`branch_gate` — политика раннера (как `_branch_chance`), персистентна
(`branch_gate` в `campaign_state`). Канон «гейт в цели ветки» **остаётся**:
множитель чинит explore, но не заменяет ограничение в argmax (строка 4
таблицы).

**Графика** (`verification/torn_plots.py`, PNG в `output/iter100/`):
`star_projection` — «цветик» (оси A,B,C,T,P по кругу): слева σ суррогата
`yield` по пулу с красным контуром истинной дыры, справа те же координаты —
seed годные/негодные, точки раундов (цвет = номер, X = промах), `x_best`,
аналитический оптимум; `cliff_slice` — `yield` vs `surface − 4`: истина,
μ±2σ суррогата, точки раундов. На срезе видно: за кромкой μ суррогата
продолжает экспоненту вверх (14+ при истине 0), а σ растёт — это и есть
приманка для explore.

**OPEN_QUESTIONS:** ~~UI для `set_branch_gate` (поле «гейт измеримости» в
форме ветки) и подсказка «в цели ветки нет гейт-отклика при наличии
пропусков в базе»~~ — **сделано в iter104 (§16.2.1.5, UI 2.1/2.4)**;
автоматический вывод гейта из паттерна MISSING (сейчас гейт объявляет
человек).

#### §16.2.1.2 Реальные бюджеты и каскад «дыра → окрестность → компонент» (iter101)

**Вопрос 1 — цена результата iter100.** Там было 60 seed + 6×4 = **84
опыта** на 3+2 оси; iter99 — 70 + 40 = 110. Это не лабораторный объём.
Замер того же мира (`CliffTruth`, гейт в цели + `set_branch_gate`, explore
0.3) на бюджетах **24 / 28 / 36 опытов** (12/16/24 seed + 4 раунда × 3), по
два rng-сида (`test_iteration101_cascade_budget.py::test_realistic_budget_*`):

| N опытов | измеримых seed | d_best / d_opt | промахи | глубина |
|---|---|---|---|---|
| 24 | 4–5 из 12 | 99.1–99.3 % | 8–25 % | ≤ 0.29 |
| 28 | 4–6 из 16 | 99.5–99.8 % | 0–8 % | ≤ 0.04 |
| 36 | 9 из 24 | 98.1–99.3 % | 0–25 % | ≤ 0.06 |

Вывод: к кромке ядро выходит на 24–28 опытах; GP при этом работает с
линейным/константным трендом (квадратичный требует n ≥ 34, `GPExpert`), и
это не мешает — кромку держит гейт в цели и множитель измеримости, а не
глобальная модель отклика.

**Вопрос 2 — каскад.** Мир `build_truth_4comp_cliff` ({A,B,C,D}×{T,P}; на
грани D=0 — бит-в-бит мир iter100; D усиливает gloss-гряду `B*C` и
сглаживает поверхность там же). Лаборатория `StagedLab`: физика фиксирована
(новые переменные отвергаются), а **приборы появляются по ходу** —
`declare_response` включает отклик истины (первая не-ручная реализация
протокола iter99). Цель `gloss≥8 max ∧ strength≥6 max ∧ surface≥4`,
`satisfy_at = 0.9`; 56 опытов на весь каскад:

| этап | операция ядра | N | потолок этапа | d_best | статус |
|---|---|---|---|---|---|
| 1 скрининг {A,B,C} | `begin_phase`, 20 seed + 3×3 | 29 | **0.805** | 0.801 | active — цель недостижима: max gloss лежит в дыре |
| 2 окрестность + прибор | `edge_region` → `move_region` restrict; `add_response(yield)`; 6 seed области + 3×3; `set_desirability(+yield)` | 44 | 0.849 | 0.847 | active |
| 3 компонент D | `add_mixture_component("D")`, 4×3 | 56 | **0.944** | 0.931 | **satisfied** |

`d_final(x_best)` по истине и финальной цели: 0.841 → 0.847 → 0.931 —
монотонно. Промахи по этапам: 0 / (2 из 6 seed области, глубина 1.25; далее
0) / 0.

**Новые примитивы (все чистые, раннера не знают):**
- `design.branches.edge_region(surrogate, thr, cands, names, width=…)` —
  окрестность кромки как бокс кандидатов с `|μ_gate − thr| ≤ width`
  (единицы гейта: «surface 4 ± 0.3»); бокс точек симплекса
  симплекс-замкнут по построению (доказательство в docstring). Полоса по
  `P(измеримо|x)` отвергнута: у GP без шума `Φ((μ−thr)/σ)` — ступенька, в
  полосе 0.15–0.85 оставалось 4 кандидата из 2000.
- `verification.branch_reference.branch_optimum_region(truth, goal, region,
  baseline)` — потолок на **области фазы** (bounds схемы раннера,
  отсутствующие компоненты = 0, оси = baseline). Без него d_best этапа
  сравнивать не с чем: глобальный оптимум лежит вне области этапа.
- `_stochastic_polish` в `branch_optimum` / `branch_optimum_region`: оптимум
  цели с обрывом лежит **на разрыве** (`yield` скачет с `y_cliff` в 0), и
  SLSQP с численным градиентом «уточнял» вниз (0.8467 → 0.8442 из той же
  точки) — измеренный d_best превышал потолок при нулевом шуме. Полировка
  derivative-free, только вверх; на мире iter100 даёт тот же `d_opt = 0.9409`.

**Что вскрылось и не решено (OPEN):**
- ~~`propose_seed` на непустой базе = `propose_augment` (maximin от
  существующих точек, iter37): после сужения области он отталкивается от
  точек, сгущённых в измеримой зоне, и **все 6 точек добора ушли в дыру**
  (gate 2.7–3.2). В тесте — `reuse_existing=False`. Добор новой области
  должен учитывать измеримость (тот же множитель, что у explore) — задача
  ядра.~~ — **закрыто в iter103 (§16.2.1.4)**: `propose_augment(...,
  feasibility=)`, критерий `d_min · P(измеримо|x)`; в каскаде seed этапа 2
  измерим 6 из 6 на всех 10 сидах.
- ~~`move_region` по process-осям … для ручной кампании с реальными
  единицами это работает как задумано~~ — **утверждение было неверным,
  закрыто в iter102 (§16.2.1.3)**: проба на ручном раннере показала, что
  точки «переезжали» вместе с границами и в синтетике, и в ручной кампании.
- Ветка с новым откликом в цели не может предлагать точки, пока у отклика
  нет суррогата (`KeyError` по контракту `branch_scores`) — порядок «сначала
  домерить, потом +цель» сейчас ответственность вызывающего.

#### §16.2.1.3 PROCESS-координаты точки — физика, не код (iter102)

**Находка (проба 16.09.2026, ручной раннер `build_setup_runner`, A,B ×
T∈[150,200], 16 точек, y = T/100).** После `restrict_bounds("T", 170, 180)`:
все 16 точек оставались в активном пуле (12 из них сняты при T вне
[170,180]); код 0.9, показывавший T=195, стал показывать T=179 при прогнозе
GP 1.95 (истина при T=179 — 1.79); `deactivate_variable("T", 170)` давал
кандидатов с T = 170.1…170.9 (`from_code` при `span=0` подставлял `span=1`).
Причина: `DataPoint.X["PROCESS"]` хранил **код [0,1]**, `point_in_region` и
`validate` проверяли лишь `код ∈ [0,1]`, а границы переинтерпретировали код —
измеренный Y приписывался другому режиму (класс рассогласования §15.0.4).
Тест iter94 `test_deactivated_axis_is_pinned_and_reversible` проходил именно
потому, что коды против реальных границ не проверялись.

**Решение — физическое значение process-оси = инвариант точки:**
- `DataPoint.X["PROCESS"]` — **реальные единицы** (°C, Гц: как выставлял
  оператор). `composite_coords(schema, point)` строит код **под схему-аргумент**
  (точка вне границ даёт код вне [0,1] — сигнал, не клип). `process_code(schema)`
  требует схему явно; `process_real()` — как хранится.
- `validate` / `point_in_region` — единый предикат `process_in_bounds(block,
  values, tol)`: `lo ≤ v ≤ hi`, допуск — доля размаха оси (на зажатой оси —
  абсолютный). Сужение process-оси теперь **исключает** точки вне области и
  **возвращает** их при обратном relax (§15.0.3.3) — как у mixture.
- `migrate_point`: `known-constant` для process кладётся реальным значением
  без code-трансформа под границы цели (стало проще; `_process_migration`
  раннера уже хранил реал).
- `VariableBlock.to_code`/`from_code` на вырожденной оси: код 0 / значение
  `lo` при любом коде (было `lo + код`).
- Раннер: `_make_point` переводит код кандидата в физику по границам ТЕКУЩЕЙ
  схемы один раз; `_to_full` и новый `_from_full_process` идут через физику
  (границы фазы после move_region ≠ границам полной схемы; при равных —
  тождество); `_branch_reference_recipe` читает `x_best` (полный вектор) через
  физику.
- Сейв: `DataPoint.to_dict` пишет маркер `process_units: "real"`; старый сейв
  без маркера конвертируется при загрузке `point_from_legacy_code(d,
  schema_of_version)` по границам **версии точки** (то же преобразование, каким
  UI показывал точку до iter102); несогласованный сейв — отказ, не догадка.
- Незафиксированный план (`setup_seed_X`, код): рядом хранится
  `setup_seed_proc_bounds` / `draft.seed_proc_bounds`; при движении
  process-границ до фиксации Y и при загрузке план **перекодируется с
  сохранением физики** (`recode_seed_plan`), строки вне новых границ считаются
  и показываются, но не клипуются и не выбрасываются (A0.6). Без сохранённых
  границ (старый черновик) план читается как есть.

**Контракт сменился явно:** тесты iter12/iter94, кодировавшие «PROCESS в коде»,
переписаны на физику (комментарии в тестах указывают на iter102). Гейт-тест:
`test_iteration102_process_real_units.py` (20 тестов: ядро, раннер,
персистентность со старым сейвом, перекодировка черновика). Регресс: 266 + 214
+ 104 тестов.

**Что это открывает для каскада §16.2.1.2:** сужение окрестности кромки по
T/P (`edge_region` уже отдаёт бокс по process-осям) теперь корректно и в
синтетике, и в ручной кампании — process-оси из бокса можно применять
`move_region`, а не только показывать текстом (сделано в iter103:
`runner.edge_box_to_deltas`).

#### §16.2.1.4 Добор области с множителем измеримости (iter103)

**Задача (OPEN §16.2.1.2).** `propose_augment` — жадный maximin от
существующих точек. После сужения области к кромке дыры existing сгущены в
измеримой зоне, и maximin, отталкиваясь от них, уводит добор в дыру: на мире
`CliffTruth` (20 seed → `edge_region` → restrict по A,B,C) голый maximin даёт
**2 измеримых из 6**, глубина промаха до 1.6.

**Решение.** `propose_augment(n, feasibility=)` / `propose_seed(...,
feasibility=)`: критерий выбора `d_min(x) · P(измеримо|x)` — тот же
множитель, что у explore-члена acquisition (iter100). `feasibility=None` и
`P ≡ 1` — бит-в-бит прежнее; `P` клипуется в [0,1]; несовпадение длины —
`ValueError`; кандидатов с `P ≥ 0.5` меньше `n` — `UserWarning` (A0.6).
Дубли исключаются маской `taken`, а не `d2 = −1` (при `P = 0` старый трюк
дал бы повторы). На пустой базе с множителем первая точка — самый измеримый
кандидат, далее тот же критерий.

Множитель без ветки: `runner.gate_feasibility_fn(response, threshold,
direction)` — из общего суррогата гейта; суррогата нет (гейт ни разу не
измерен) — `RuntimeError` с покрытием, не молчаливое `≡ 1`.

Замер (`test_iteration103_feasible_augment.py`): с множителем **6 из 6**
измеримы, `gate − thr ≥ +0.05` (кромка рядом, но с годной стороны); `yield`
снят на 6 из 6 точек добора. Каскад iter101 переведён на штатный путь
(`propose_seed(feasibility=)` вместо обхода `reuse_existing=False`): seed
этапа 2 без единого промаха на всех 10 сидах (7…41); раньше 2–5 промахов из
6, а на сиде 19 все 6 уходили в дыру и `yield` не измерялся вовсе.

**Что вскрылось (OPEN):**
- ~~**После restrict суррогат гейта забывает дыру.** `move_region` →
  `refit_if_possible` учит все суррогаты на АКТИВНОМ пуле; точки дыры,
  выпавшие из области, из обучения гейта пропадают. Точность `P(измеримо)`
  на пуле кандидатов суженной области падает с 1.00 до 0.79 (20 → 9 точек);
  множитель из «послерестриктного» суррогата даёт 4 из 6 вместо 6 из 6.
  Рабочий приём сейчас — строить множитель **до** сужения.~~ — **закрыто в
  iter104 (§16.2.1.5)**: область обучения по отклику; гейт учится на всей
  истории, точность после restrict = точности до (≥ 0.95), добор с
  «послерестриктным» множителем 6 из 6.
- Этап 3 каскада (12 точек на поиск D) — лотерея сида: satisfied в 5 из 10 с
  множителем и 6 из 9 без; различие не от добора (этап 2 везде лучше или
  равен), а от бюджета этапа 3. Тест фиксирует сид 13.
- `edge_box_to_deltas`: бокс по T/P теперь применим `move_region` в физике;
  в каскаде process-оси по-прежнему не сужаются — истина гейта вогнута по
  T, полоса кромки покрывает почти всю ось, и бокс по T неинформативен.

#### §16.2.1.5 Область обучения суррогата и проектный гейт (iter104)

**Задача (OPEN §16.2.1.4).** После `move_region` (restrict к кромке дыры)
все суррогаты переобучаются на активном пуле. Для гейта это потеря не
«части данных», а **всего отрицательного класса**: точки дыры несут
измеренный гейт и MISSING по зависимым откликам, и именно они выпадают из
области. В тестах обходили, строя множитель до сужения и держа его в
переменной; в UI между сужением и добором — reruns Streamlit, так не выйдет.

**Разбор (обсуждение 24.09.2026).** Для GP «лишняя» измеренная точка вне
области почти никогда не вредит предсказанию внутри при **той же физике**
снаружи: ядро затухает с расстоянием, а точки у границы держат σ на кромке
честной. Вред приходит через **общие гиперпараметры**: один тренд Шеффе,
один вектор lengthscale, один уровень шума на всю выборку. Если снаружи
отклик живёт по другому закону (другое сырьё/оборудование/режим ниже
желатинизации — как раз причины `restrict` с intent `physical_constraint`),
тренд искривится, lengthscale поедет: снаружи «рванее» → σ внутри раздуется
(лишний explore), снаружи глаже → σ схлопнется и GP «уверенно врёт» (класс
iter30). Критерий «измерено ⇒ применимо» неполон: недостающее условие —
**непрерывность режима**. Различает либо человек (intent движения границ
уже в журнале `_region_moves`), либо данные — LOO на активных точках.

Отдельно: **новый компонент — не смена физики**. Старые точки мигрируют на
грань `D=0` с `known-constant(0.0)` — это точная запись того, что было в
пробирке; суррогат знает функцию на грани и экстраполирует внутрь с честно
большой σ, старые точки — опора, а не помеха. «Переделывать измерения» не
нужно. Три случая рядом: сужение области (та же физика, точки за краем →
оставить в обучении), новый компонент/ось (та же физика, точки на грани →
уже сделано `known-constant`), смена режима при тех же координатах (другая
функция → отделять: блок-эффект или отдельная область обучения по тегу).

**Решение (вариант 1 из трёх — явное объявление + LOO-подсказка).**
- `runner.set_training_scope(response, "active" | "history")` /
  `training_scope` / `training_scopes` — **область обучения по отклику**,
  политика раннера, персистентна (`training_scope` в `campaign_state`).
  `history` = вся история проекта, мигрированная к текущей схеме
  (`_migrated_points(include_outside=True)`), включая точки вне области.
  `X`/`Y` (активный пул) **не меняются**: измеренный рекорд ветки обязан
  лежать в области. `surrogate_coverage` получает ключ `scope` (только при
  `history`) и считает `n_base`/`n_train` по обучающей выборке.
- `runner.set_project_gate(response, thr, dir)` / `project_gate` /
  `project_feasibility_fn` — **проектный гейт** (уровень кампании): нужен
  для добора без ветки и служит дефолтом веткам без собственного
  (`effective_branch_gate`: собственный > проектный > None; `source` в
  ответе). `propose_seed(feasibility=None)` при объявленном проектном гейте
  берёт множитель из него; `feasibility=False` — явно без множителя;
  необученный гейт — `RuntimeError`, не `≡ 1`.
- **Объявление гейта** (`set_project_gate` или `set_branch_gate`)
  переводит гейт-отклик на область `history` — гейт описывает физику
  измеримости, а не область интереса. Снятие гейта область не возвращает
  (отдельное решение).
- `runner.training_scope_diagnostics(response)` — read-only LOO на
  **активных** точках: модель на активе vs модель на истории
  (`GPExpert.loo_scores` — закрытая форма R&W §5.4.2 при фиксированных
  гиперпараметрах, сверена с честным refit). `history_helps=True` — снаружи
  та же физика, историю можно включить; `False` — снаружи другой режим,
  оставить актив; `None` — сравнивать нечего. Это подсказка технологу, не
  автопереключение (при 8–12 активных точках LOO шумный — автовыбор дал бы
  «мигание» модели между раундами).

**Замер (`test_iteration104_training_scope.py`, 12 тестов):** тот же мир и
сид, что в iter103, два раннера — без гейта (`active`) и с проектным
гейтом (`history`), один и тот же restrict к кромке. Точность
`P(измеримо)` на пуле суженной области: до сужения 1.00; после — `active`
< 1.00 (факт iter103 воспроизводится), `history` = до сужения (≥ 0.95).
Добор с множителем, построенным **после** сужения: `history` — 6 из 6
измеримы, `gate − thr > −0.1`, `yield` снят на ≥ 5 из 6. Без сужения
`history` ≡ `active` бит-в-бит. LOO-диагностика на гладкой истине: та же
физика снаружи → `history_helps=True`; сдвиг режима снаружи (`+40` и другая
форма) → `False`. Регресс iter98/99/100/103/94/20/43/19 — без изменений.

**OPEN (режимный сдвиг):** смена физики при тех же координатах (партия
сырья, оборудование, режим) сейчас не моделируется: у точки уже есть
`origin_tag.campaign`/`block`, но обучение по тегу и модель сдвига между
режимами не сделаны. Кандидаты: (а) блок-эффект как аддитивный сдвиг
(иерархический GP — дёшево, если разница в уровне, а не в форме);
(б) область обучения `campaign:<label>` — расширение `training_scope` без
переделки (старые точки как слабый prior). Различать (а)/(б) той же
LOO-диагностикой на точках нового режима. Без этого технолог обязан
переключать область на `active` при смене режима вручную.

### §16.3 Мультицелевая ветка с диапазонами

**Тезис.** Ветка — это НАБОР целей, а не одна цель. ТЗ ЯВНО снимает ограничение
«одна ветка — одно целевое значение»: в одной ветке сосуществуют несколько
свойств, каждое со своим видом десирабилити (`min`/`max`/`target`), своим
диапазоном `[low, high]` (и `target` для target-типа) и своим весом.

**Контракт редактора целей ветки (поверх уже реализованного
`CampaignController`):**
- `set_desirability(branch, response, spec)` — добавить/заменить цель над ЛЮБЫМ
  измеряемым откликом (новая цель ⇒ роль `OPTIMIZED`, §16.0);
- `set_weights(branch, {response: weight})` — веса целей (геом-среднее d_i);
- `delete_goal(branch, response)` — удалить цель; **запрет удаления последней**
  (объектив ветки должен существовать — иначе re-score/argmax не определены).
- Каждая правка обратима (undo-стек §7 `CampaignController`) и НЕ трогает
  измеренную правду (И-1): меняется только ОЦЕНКА (`_rescore` под новый объектив)
  и показывается смещение рекомендации x* (X→Y).

**Инвариант XOR ролей (§16.0).** Множество целей не нарушает однозначность роли:
приоритет `OPTIMIZED > PRICE_INPUT > REFERENCE` разрешает совмещение «ρ — цель И
питает цену» (роль ρ = OPTIMIZED, σ_ρ-ценовой канал занулён, Гр-1).

**Гейт-тест (§16.6).** Ветка с ≥3 целями разных видов/диапазонов/весов; add/edit/
delete цели меняют d_best и x* предсказуемо; удаление последней цели — отказ.

### §16.4 Рабочий стол ветки — добор точек (внутриветочный цикл)

**Тезис.** «Зайдя в ветку», пользователь обязан иметь полный цикл добора: от
предложения точек до внесения результатов эксперимента и переобучения.

**Цикл (один раунд ветки, поверх `run_branch_round`):**

```
1. ПРЕДЛОЖИТЬ   acquisition/argmax по ОБЛАСТИ (симплекс+process):
               acq(x) = (1-explore)·d_overall(ŷ(x)) + explore·σ̄_n(x)
               → N точек-кандидатов (diverse, min_dist).
2. ИЗМЕРИТЬ     внести лабораторные Y по ВСЕМ P свойствам
               (демо-режим: синтетический оракул; боевой: ручной ввод).
3. ДОЛИТЬ       точки → общая база с origin-тегом branch:{id} (И-1, без копий).
4. ПЕРЕОБУЧИТЬ  суррогаты MoE/GP на расширенной базе (одна модель на проект).
5. ПЕРЕСЧИТАТЬ  x*/d_best ветки; обновить spent/budget/status.
6. СТОП §4      двойной критерий: технический (Δd≥ε ∧ d<ceil) И экономический
               (EI_price·V·H > c_exp); stop_reason ∈ {ceil_reached,
               stagnation, not_economical}; при упоре в границу — триада §6.
```

**Инварианты (binding):**
- **A0.6 — мерим только по явной команде.** Шаг 2 (внесение Y / запуск
  демо-оракула) — действие пользователя, система ничего не измеряет молча.
- **И-1 — история цела.** Добор только ДОБАВЛЯЕТ; прогон раунда запечатывает дно
  undo-стека (измеренную правду откатить нельзя, §7).
- **Общая база, origin-теги.** Новая точка меряется по ВСЕМ свойствам и
  дописывается в общую базу (`runner.origin = branch:{id}`), как в каноне §5/§12.

**Гейт-тест (§16.6).** Вход в ветку → раунд добора: база выросла на N, у точек
origin-тег ветки, d_best не убыл (монотонность), суррогаты переобучены, стоп
возвращает легальный `stop_reason`.

### §16.5 Замкнутый цикл кампании (сквозной контур)

Сшивка §16.2–§16.4 + ранее реализованных §16.0/§6 в единый замкнутый цикл —
это и есть отсутствовавший «понятный цикл от добавления параметров и целей до
повторного прохождения с новыми точками и результатами».

```
        ┌─────────────────────────────────────────────────────────────┐
        ▼                                                               │
  СТАРТ: минимальный набор параметров (например A,B) + минимум целей    │
        │                                                               │
        ▼                                                               │
  §16.4 РАУНДЫ ДОБОРА ветки: предложить → измерить → долить →           │
        переобучить → x*/d_best → §4 стоп                               │
        │                                                               │
        ├─ d растёт, цель не достигнута ──────────────────────────┐     │
        │                                                         │     │
        ▼                                                         │     │
  §4/§6 УПОР В ПОТОЛОК или НОВАЯ ЦЕЛЬ КЛИЕНТА:                     │     │
    • soft-граница упёрта → денежная ТРИАДА §6 (двигать? A0.6)    │     │
    • нужен новый компонент/режим/отклик → §16.2 эволюция схемы   │     │
    • нужна новая/иная цель или диапазон → §16.3 правка целей     │     │
        │                                                         │     │
        ▼                                                         │     │
  §16.2 МИГРАЦИЯ старых точек по явной политике (И-1, A0.6)       │     │
        │                                                         │     │
        └──────────────► повтор раундов добора (назад в §16.4) ◄──┘     │
        │                                                               │
        ▼                                                               │
  ЦЕЛЬ ДОСТИГНУТА (satisfied) ИЛИ невыгодно (not_economical) ───────────┘
        стоп ветки; spawn сиблинга §8 при новой продуктовой линии
```

**Ключевые свойства контура:**
- цикл **повторяемый**: расширение параметров/целей возвращает в раунды добора на
  ТОЙ ЖЕ общей базе (миграция, не пересборка проекта);
- решения о расширении/движении границ — **предложения пользователю** (A0.6,
  триада §6), не автоматические;
- spawn ветки (§8) наследует роли/намерение и работает на общем пуле — новая
  продуктовая линия не плодит вторую модель физики.

### §16.6 Карта реализации и гейты тестов (для следующей сессии)

Статус (обновлено 2026-07-01): **логика + гейт-тесты §16.2–§16.5 РЕАЛИЗОВАНЫ**
(57 passed вместе с регрессией iteration16/18); остаётся **UI** и проброс в
`doe-introspect`/assistant (E+). Порядок по канону соблюдён: логика + unit-тест,
UI — отдельным шагом.

Отступление от «предполагаемого места»: фасад операций схемы §16.2
(`add_process_var`/`add_mixture_component`/`add_response`/`relax_bounds`/
`restrict_bounds`) приземлён в `campaign.CampaignController` (тонкая обёртка над
`runner.augment_phase_*`/`move_region`/`evolve_schema` с проверкой A0.6), а НЕ
методами самого `MixtureProcessRunner` — это второй из двух допустимых слотов
карты («campaign.CampaignController»). Так эволюция схемы, редактор целей и раунды
добора собраны в одном фасаде кампании; ядро не тронуто (анти-объём §16.6). Карта
привязки к коду:


| § | Что вводим | Где (предполагаемо) | Гейт-тест |
|---|---|---|---|
| §16.2 | фасад операций схемы кампании (`add_mixture_component`/`add_process_var`/`add_response`/`relax|restrict_bounds`) поверх `evolve_schema`/`augment_phase_*`/`move_region` с явной миграцией | `MixtureProcessRunner` (+ тонкая обёртка), `campaign.CampaignController` | новый `test_iteration19_campaign_schema.py` |
| §16.3 | мультицелевой редактор ветки (несколько целей, диапазоны, веса) — вывод в UI уже реализованного `set_desirability/set_weights/delete_goal` | `campaign.CampaignController` (есть) + `streamlit_app.render_branches` (правка: убрать «один prop») | `test_iteration19_branch_multigoal.py` |
| §16.4 | рабочий стол ветки: предложить N → ввести Y → долить → переобучить → x*/стоп §4 | `run_branch_round` (есть) + UI-«рабочий стол» | `test_iteration19_branch_workbench.py` |
| §16.5 | сквозной сценарий замкнутого цикла (интеграционный) | поверх §16.2–§16.4 | `test_iteration19_campaign_loop.py` |
| — | прокинуть параметры/цикл в `doe-introspect` (MCP) и assistant | `src/mcp/*`, `apps/assistant.py` | расширить MCP-тесты |

**Чего делать НЕ нужно (анти-объём):** заводить вторую модель физики на ветку
(канон §5/§12 — одна модель на проект); вводить второй критерий остановки (двойной
стоп §4 — единственный); удалять/урезать историю при миграции (И-1).
